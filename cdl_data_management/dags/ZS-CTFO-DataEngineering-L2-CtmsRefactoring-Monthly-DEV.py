...
# Name of the Dag
DAG_NAME = "ZS-CTFO-DataEngineering-L2-CtmsRefactoring-Monthly-DEV"
PROCESS_ID = 4000
FREQUENCY = "monthly"
...
# #<---------------------->
CtmsSite = PythonOperator(
    task_id="usl_study_site_investigator",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "usl_study_site_investigator"},
    dag=dag)

CtmsCountry = PythonOperator(
    task_id="usl_study_country",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "usl_study_country"},
    dag=dag)

CtmsStudy = PythonOperator(
    task_id="usl_study",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "usl_study"},
    dag=dag)

CtmsSubject = PythonOperator(
    task_id="usl_subject",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "usl_subject"},
    dag=dag)
...
CtmsSite.set_upstream(launch_ddl_creation)
CtmsCountry.set_upstream(launch_ddl_creation)
CtmsStudy.set_upstream(launch_ddl_creation)

CtmsSubject.set_upstream(CtmsSite)
CtmsSubject.set_upstream(CtmsCountry)
CtmsSubject.set_upstream(CtmsStudy)


publish.set_upstream(CtmsSubject)
