...
# Name of the Dag
DAG_NAME = "ZS-CTFO-DataEngineering-L2-Fact-A1-Reporting-Monthly-DEV"
PROCESS_ID = 3000
FREQUENCY = "monthly"
...
RptSubjectSiteStudyDetails = PythonOperator(
    task_id="RptSubjectSiteStudyDetails",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID,"frequency":FREQUENCY, "step_name": "RptSubjectSiteStudyDetails"},
    dag=dag)
...
RptIhmeCountyDiversity= PythonOperator(
    task_id="RptIhmeCountyDiversity",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "RptIhmeCountyDiversity"},
    dag=dag)
...
dRptCountry = PythonOperator(
    task_id="dRptCountry",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "dRptCountry"},
    dag=dag)
...
#RptCountryFootprint.set_upstream(launch_ddl_creation)
RptSubjectSiteStudyDetails.set_upstream(launch_ddl_creation)
RptPatientDetails.set_upstream(launch_ddl_creation)
RptPatientDiversity.set_upstream(launch_ddl_creation)
RptIhmeCountyDiversity.set_upstream(launch_ddl_creation)
RptFiltersTrial.set_upstream(RptFiltersSetupAll)
RptFiltersSiteInv.set_upstream(RptFiltersTrial)

dRptCountry.set_upstream(launch_ddl_creation)

batch_id_maintain_future.set_upstream(RptSubjectSiteStudyDetails)
...
#batch_id_maintain_future.set_upstream(RptCountryFootprint)
...
batch_id_maintain_future.set_upstream(RptPatientDiversity)
batch_id_maintain_future.set_upstream(RptDrugDetails)
batch_id_maintain_future.set_upstream(dRptCountry)
batch_id_maintain_future.set_upstream(RptPatientDetails)
batch_id_maintain_future.set_upstream(RptIhmeCountyDiversity)
batch_id_maintain_future.set_upstream(RptSiteStudyDetails)
batch_id_maintain_future.set_upstream(RptCountry)
batch_id_maintain_future.set_upstream(RptTrialUniverse)
publish.set_upstream(batch_id_maintain_future)
