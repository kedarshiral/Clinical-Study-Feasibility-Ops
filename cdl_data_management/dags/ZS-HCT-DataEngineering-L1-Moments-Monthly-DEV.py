...
FILE_MASTER_IDS = ["1601", "1602", "1603", "1604", "1605"]

PROCESS_ID = 1600

DAG_NAME = "ZS-HCT-DataEngineering-L1-Moments-Monthly-DEV"
...
start = PythonOperator(
    ...
    op_kwargs={"email_type": "batch_status",
...
copy_files = PythonOperator(
    task_id="copy_files",
    python_callable=dagutils.call_moments_hct_files_copy, provide_context=True,
    op_kwargs={"dag_name": DAG_NAME, "process_id": PROCESS_ID},
    dag=dag)

qc_checks = PythonOperator(
    task_id="qc_checks",
    python_callable=dagutils.call_moments_hct_files_qc, provide_context=True,
    op_kwargs={"dag_name": DAG_NAME, "process_id": PROCESS_ID},
    dag=dag)

emr_launch.set_upstream(start)
copy_files.set_upstream(emr_launch)
qc_checks.set_upstream(copy_files)
...
for file_master_id in FILE_MASTER_IDS:
    ...
    pre_landing = PythonOperator(
        ...
        op_kwargs={"file_master_id": file_master_id,
                   "dag_name": DAG_NAME, "process_id": PROCESS_ID},
        dag=dag)
    ...
    if (file_master_id == "1601"):
        # Since this connection is with 1601 file only.
        column_standardization = PythonOperator(
            task_id="column_standardization_" + str(file_master_id),
            python_callable=dagutils.call_moments_hct_column_standardization, provide_context=True,
            op_kwargs={"file_master_id": file_master_id,
                       "dag_name": DAG_NAME, "process_id": PROCESS_ID},
            dag=dag)

        file_standardization = PythonOperator(
            task_id="file_standardization_" + str(file_master_id),
            python_callable=dagutils.call_moments_hct_file_standardization, provide_context=True,
            op_kwargs={"file_master_id": file_master_id,
                       "dag_name": DAG_NAME, "process_id": PROCESS_ID},
            dag=dag)
        
        column_standardization.set_upstream(qc_checks)
        file_standardization.set_upstream(column_standardization)
        pre_landing.set_upstream(file_standardization)

    else:
        pre_landing.set_upstream(qc_checks)
