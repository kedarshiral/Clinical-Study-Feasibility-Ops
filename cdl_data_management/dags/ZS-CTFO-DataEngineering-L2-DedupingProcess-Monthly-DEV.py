...
# Name of the Dag
DAG_NAME = "ZS-CTFO-DataEngineering-L2-DedupingProcess-Monthly-DEV"
PROCESS_ID = 1000
FREQUENCY = "monthly"
...
AllSiteDedupeioProcessAACT = PythonOperator(
    task_id="AllSiteDedupeioProcessAACT",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "AllSiteDedupeioProcessAACT"},
    dag=dag)

AllSiteDedupeioProcessTASCAN = PythonOperator(
    task_id="AllSiteDedupeioProcessTASCAN",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "AllSiteDedupeioProcessTASCAN"},
    dag=dag)

AACTGoldenRecord = PythonOperator(
    task_id="AACTGoldenRecord",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "AACTGoldenRecord"},
    dag=dag)
...
AllSiteDedupeioProcessAACT.set_upstream(AllSiteDedupeDataPrep)
AllSiteDedupeioProcessAACT.set_upstream(AllTrialDedupeDataPrep)
AllSiteDedupeioProcessAACT.set_upstream(AllInvDedupeDataPrep)
AllSiteDedupeioProcessTASCAN.set_upstream(AllSiteDedupeioProcessAACT)
AACTGoldenRecord.set_upstream(AllSiteDedupeioProcessTASCAN)

AllSiteDedupeioProcess.set_upstream(AACTGoldenRecord)
