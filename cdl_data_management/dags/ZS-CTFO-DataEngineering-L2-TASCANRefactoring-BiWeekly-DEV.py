...
# Name of the Dag
DAG_NAME = "ZS-CTFO-DataEngineering-L2-TASCANRefactoring-BiWeekly-DEV"
PROCESS_ID = 20000
FREQUENCY = "BiWeekly"
...
# #<---------------------->
tascan_investigator = PythonOperator(
    task_id="tascan_investigator",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "tascan_investigator"},
    dag=dag)

tascan_site = PythonOperator(
    task_id="tascan_site",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "tascan_site"},
    dag=dag)

tascan_trial = PythonOperator(
    task_id="tascan_trial",
    python_callable=dagutils.call_job_executor,
    op_kwargs={"process_id": PROCESS_ID, "frequency": FREQUENCY, "step_name": "tascan_trial"},
    dag=dag)
...
tascan_investigator.set_upstream(launch_ddl_creation)
tascan_site.set_upstream(launch_ddl_creation)


tascan_trial.set_upstream(tascan_investigator)
tascan_trial.set_upstream(tascan_site)

publish.set_upstream(tascan_trial)
