...
class CreateStructuredFile:
    ...
    def execute_spark_query(self):
        ...
        try:
            ...
            # if self.data_source == "who":
            #     current_time = datetime.strftime(
            #         datetime.now(), "%Y%m%d%H%M%S"
            #     )
            #     temp_parquet_output_path = path.join(
            #         self.temp_s3_path, self.data_source + "_" + current_time
            #     )
            #     logging.info("Adding disease area column to dataframe")
            #     raw_data_frame = raw_data_frame.withColumn("disease_area_name", lit(self.disease_name))
            #     logging.info("Writing dataframe to s3 in parquet format")
            #     raw_data_frame.coalesce(1).write.format("parquet").mode("append").save(
