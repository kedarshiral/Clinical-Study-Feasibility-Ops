"""
  ...
  Pre-requisites      :
  Last changed on     :   25th April 2023
  Last changed by     :   Shivangi Jain
...
SCHEMA_FILE_NAME = "pharmaprojects_schema.json"
SQL_FILE_NAME = "pharmaprojects.sql"
...
class DataIngestionAdapter:
    ...
    def call_create_structured_file(
        ...
        try:
            ...
            print("emr path *****************", emr_code_path)
            emr_region_name = application_config["adapter_details"]["generic_config"]["aws_region"]
            s3_bucket_name = application_config["adapter_details"]["generic_config"]["s3_bucket_name"]
            ...
            ADAPTOR_LOCATION = CommonConstants.EMR_CODE_PATH + "/code/adaptor/pharmaprojects "
    ...
    def main(self, input_dict):
        ...
        try:
            ...
            logging.info(status_message)
            self.data_source = "pharmaprojects"
            ...
            for payload_json in payload_json_list:
                ...
                for sub_payload in payload_json:
                    ...
                    cluster_id_query = (
                        "SELECT a.cluster_id FROM (select * from {audit_db}.log_adapter_smry where cluster_id is not null and trim(cluster_id) not in (' ')) a "
                        " where a.payload_id ='{payload_id}' order by a.end_time desc limit 1")

                    output_query = cluster_id_query.format(
                        audit_db=self.audit_db,
                        payload_id=payload_id
                    )

                    logging.info("Query to fetch cluster_id: %s", str(output_query))
                    cluster_id_fetched = MySQLConnectionManager().execute_query_mysql(output_query, False)
                    cluster_id_fetched = cluster_id_fetched[0]['cluster_id']
                    logging.info("Query output of cluster id : %s", str(cluster_id_fetched))

                    if cluster_id_fetched == '' or len(cluster_id_fetched) == 0:
                        logging.info("Launching cluster as cluster id is empty ...")
                        ...
                        try:
                            response = client.describe_cluster(ClusterId=cluster_id_fetched)
                            status = response['Cluster']['Status']['State']
                            if status in ('STARTING', 'BOOTSTRAPPING', 'RUNNING', 'WAITING'):
                                print("Cluster already launched!")
                                cluster_id = cluster_id_fetched
                            else:
                            ...
                            logging.info("Cluster not found on EMR. Launching cluster ...")
                            self.cluster_id = self.call_launch_cluster(data_source_name=self.data_source)
                            self.cluster_launch_flag = "Y"
                            cluster_id = self.cluster_id
                    ...
                    EMR_CODE_PATH=CommonConstants.EMR_CODE_PATH
                    trigger_data_cmd = "cd " + str(EMR_CODE_PATH) + "/code/adaptor/pharmaprojects /; python3 " \
                                       "PharmaprojectsAdapter.py PHARMA " + " " + str(batch_id) + " " + str(payload_id) \
                                       + " " + str(FILE_ABS_PATH) + " " + str(cluster_id)
                    response, output_string = SystemManagerUtility().fetch_file_name(cluster_id, trigger_data_cmd)
                    print("here is output string: ", output_string)
