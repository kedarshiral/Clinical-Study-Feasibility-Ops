...
Last changed on     : 28th February 2019
...
CONFIG = json.loads(open(CONFIGURATION_FILE, 'r').read())
...
class SqoopIngestor(object):
    ...
    def __init__(self, input_json, execution_context=None):
        try:
            ...
            # self.tag_key_4 = CONFIG.get("CLUSTER_CONFIG", "TAGS_KEY_4")
            # self.tag_value_4 = CONFIG.get("CLUSTER_CONFIG", "TAGS_VALUE_4")
            self.region_name = CONFIG["adapter_details"]["generic_config"]["aws_region"]
    ...
    def input_validation(self, input_values):
        ...
        validated_json["columns"] = columns
    ...
    def ingest_data(self, ingestion_input):
        ...
        try:
            ...
            columns = ingestion_input["columns"]
                ...
                if not last_value:
                        ...
                        for table in table_names:
                            ...
                            if output_file_type == SqoopUtilityConstants.CSV:
                                ...
                                sqoop_command = sqoop_command + " --connect jdbc:" + db_type + "://" + \
                                                ...
                                                + " --username " + str(user_name) + " --password " + \
                                                str(password) + " --table " + str(table) + \
                                                " --columns " + columns_list + " --fields-terminated-by , " + "--escaped-by \\\\ " \
                                                + " --num-mappers 1" \
                                                + " --delete-target-dir" + " --target-dir " + self.hdfs_temp_path \
                                                + " --as-parquetfile" + " " + params
                                ...
                                logger.info("********************************************",
                                            extra=self.EXECUTION_CONTEXT.get_context())
                                logger.info("sqoop Command", extra=self.EXECUTION_CONTEXT.get_context())
                                self.sqoop_command_executor(sqoop_command=sqoop_command, table=table)
                                file_name = self.fetch_filename()
                                target_path = target_path + file_name
                                self.file_transfer(file_name=file_name, target_path=target_path)
                                print("table_names:*****", table_names)
                        logger.info("Successfully imported tables - {}".format(','.join(table_names)))
                        # logger.debug("Not terminating cluster!")
                        return self.status
                    if query:
                        if output_file_type == SqoopUtilityConstants.CSV:
                            sqoop_command = sqoop_command + " --connect jdbc:" + db_type + "://" + \
                    ...
                    if table_names and where:
                        ...
                        if output_file_type == SqoopUtilityConstants.CSV:
                            ...
                            logger.info("Executing sqoop command for table: %s" % table_names[0],
                                        extra=self.EXECUTION_CONTEXT.get_context())
                            logger.info("******************", extra=self.EXECUTION_CONTEXT.get_context())
                            logger.info(sqoop_command, extra=self.EXECUTION_CONTEXT.get_context())
                            self.sqoop_command_executor(sqoop_command=sqoop_command, table=table_names[0])
                            file_name = self.fetch_filename()
                            target_path = target_path + file_name
                            self.file_transfer(file_name=file_name, target_path=target_path)
        ...
        try:
            if self.cluster_id:
                ...
                if cluster_status == SqoopUtilityConstants.CLUSTER_STATUS_WAITING:
                    ...
                    response = SystemManagerUtility().execute_command(self.cluster_id, sqoop_command)
    ...
    def file_transfer(self, file_name, target_path):
        try:
            ...
            # response = ssh_object.file_transfer(command=command, host=master_ip_address)
            response = SystemManagerUtility().execute_command(self.cluster_id, command)
            if response:
                logger.info("File copied from HDFS to S3",
                            extra=self.EXECUTION_CONTEXT.get_context())
                self.status[0]["importStatus"] = response
                logger.info(response, extra=self.EXECUTION_CONTEXT.get_context())
                self.status[0]["file_transferred_path"] = target_path
                logger.info(str(self.status), extra=self.EXECUTION_CONTEXT.get_context())
            else:
                logger.error("File transfer failed", extra=self.EXECUTION_CONTEXT.get_context())
                self.status[0]["file_transferred_path"] = "FAILED"
        except Exception as exception:
    ...
    def fetch_filename(self):
        ...
        try:
            ...
            response, result_string = SystemManagerUtility().fetch_file_name(self.cluster_id, command)
