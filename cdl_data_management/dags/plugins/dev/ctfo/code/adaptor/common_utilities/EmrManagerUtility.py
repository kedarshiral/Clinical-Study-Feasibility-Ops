...
def terminate_adapter(source_name=None, **kwargs):
    try:
        query_str = (
            "select cluster_id from {audit_db}.{LOG_DATA_ACQUISITION_SMRY_TABLE} where "
            "lower(trim(source_name)) = lower(trim('{source_name}')) "
            "and lower(trim(status)) = lower(trim('{status}')) and cycle_id in (select max(cycle_id) from "
            "{audit_db}.{LOG_DATA_ACQUISITION_SMRY_TABLE} where "
            "lower(trim(source_name)) = lower(trim('{source_name}')) and lower(trim(status = '{status}')) )"
        )
        query = query_str.format(
            audit_db=CommonConstants.AUDIT_DB_NAME, status=CommonConstants.RUNNING_STATE,
        ...
        query_str = (
            ...
            " end_time = '{end_time}' where cluster_id = '{cluster_id}'"
        )
        update_query = query_str.format(
            audit_db=CommonConstants.AUDIT_DB_NAME, status=CommonConstants.STATUS_SUCCESS,
            LOG_DATA_ACQUISITION_SMRY_TABLE=CommonConstants.LOG_DATA_ACQUISITION_SMRY_TABLE,
        ...
        error_message = "No Cluster ID's are running"
...
class EmrManagerUtility:
    ...
    def __init__(self):
        ...
        self.configuration = json.load(open(application_conf))
    ...
    def launch_emr_cluster(self, event, region_name, process_id=None):
        ...
        try:
            ...
            cluster_name = event['Name'] + "_" + process_id
            if (validation_status[STATUS_KEY] == SUCCESS_KEY):
                ...
                result = client.run_job_flow(
                    ...
                    SecurityConfiguration=event['SecurityConfiguration'],
