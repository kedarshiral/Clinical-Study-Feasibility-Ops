...
# -*- coding: utf-8 -*-
"""
Module for Log Setup
"""
__author__ = 'ZS Associates'

# ################################# Module Information ############################################
#   Module Name         : logsetup
#   Purpose             : Setup the environment and assign value to all the variables required for
#                         logging
#   Pre-requisites      : Config variables should be present in log_setup json file
#   Last changed on     : 14 November 2017
#   Last changed by     : Amal Kurup
#   Reason for change   : Ad
# #################################################################################################

# Library and external modules declaration
import sys
sys.path.insert(0,'/home/airflow/common_components/python3_CC/code')
import logging
...
def get_logger(file_name=None, append_ts=True, timed_rotating_fh=False):
    """
    Module to get logger
    :param file_name:
    :param append_ts:
    :param timed_rotating_fh:
    :return:
    """
    ...
    logstash_private_host_name = configuration.get_configuration([MODULE_NAME,
                                                                  "logstash_private_ip"])
    ...
    logstash_logging_level = configuration.get_configuration([MODULE_NAME,
                                                              "logstash_logging_level"])
    timed_rotating_file_suffix = configuration.get_configuration([MODULE_NAME,
                                                                  "timed_rotating_file_suffix"])
    ...
    if not logger_obj.handlers:
        ...
        if log_formatter_keys is not None:
            # if log_formatter_keys exists and its not a list or blank list default formatter
            # will be created
            if len(log_formatter_keys) == 0 or not isinstance(log_formatter_keys, list):
                formatter_string = "'%(asctime)s - %(levelname)s - %(message)s '"
            else:
                # when log_formatter_keys in json contains keys, formatter using those keys
                # will be created
        ...
        if file_handler_flag == "Y":
            ...
            if file_handler_path != "" and file_handler_path is not None:
                ...
                # log file will be created on path where gui_server_code runs
                file_log_path = file_name

            if timed_rotating_fh:
                file_handler = TimedRotatingFileHandler(file_log_path,
                                                        when=timed_rotating_file_handler_cycle,
                                                        interval=
                                                        timed_rotating_file_handler_interval)
        ...
        console_handler_flag = configuration.get_configuration([MODULE_NAME,
                                                                "console_handler_flag"])
