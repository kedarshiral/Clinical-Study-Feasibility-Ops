...
sender_email = "zs.clinical@zs.com"
recipient_list = [
...
def make_dw_mail(dw_df, sender, reciever, subject, process_id):
    print()
    # msg = MIMEMultipart()
    email_from = sender
    email_to = reciever
...
def make_ingestion_mail(adapter_df, ingestion_df, file_count_df, dqm_df, athena_df, sender, reciever, subject,
                        process_id):
    print("adapter_df-", adapter_df, "ingestion_df-", ingestion_df, "file_count_df-", file_count_df, "dqm_df-", dqm_df,
          "athena_df-", athena_df)
    html_mesage = """\
    <html>
      <head></head>
...
class OFRDataValidationUtility(object):
    def __init__(self):
        ...
        self.cycle_status = CommonConstants.STATUS_SUCCEEDED
    ...
    def dw_run_time(self, process_id, **kwargs):
        try:
            ...
            msg = make_dw_mail(res_df, email_sender, email_recipient_list, email_subject, process_id)
    ...
    def ingestion_validation(self, process_id, **kwargs):
        ...
        msg = make_ingestion_mail(adapter_df, ingestion_df, file_count_df, dqm_df, athena_validation_df, email_sender,
                                  email_recipient_list, email_subject, process_id)
        ses = boto3.client("ses", region_name=self.region)
        # And finally, send the email
        # body = "Hello Team ,\n\nPlease Check the validation results"
        # attachment_name = 'validation_records.csv'
        print("Email Sender retrieved from Configurations is: " + str(email_sender))
        email_recipient_list = self.configuration.get_configuration(
            [CommonConstants.ENVIRONMENT_PARAMS_KEY, "delta_email_type_configurations", "ses_recipient_list"])
        sender = "zs.clinical@zs.com"
        ...
        try:
            ...
            for process_id in process_id_list:
                ...
                sqlquerycurrent = "select max(cycle_id) as curr_cycle_id from " \
                                  ...
                                  + " where workflow_id = ( select max(workflow_id) from " \
                                  + audit_information + ".log_cycle_dtl" \
                                  + " where process_id = " + str(process_id) + " and cycle_status = 'SUCCEEDED')" \
                                  + " and process_id = " + str(
                    process_id) + " ;"

                sqlqueryprev = "select max(cycle_id) as prev_cycle_id from " \
                               + audit_information + ".log_cycle_dtl where workflow_id  =  (select max(workflow_id) from " \
                               + audit_information + ".log_cycle_dtl where cycle_status = 'SUCCEEDED' and process_id = " \
                               + str(process_id) + " and workflow_id != (select max(workflow_id) from " \
                               + audit_information + ".log_cycle_dtl where cycle_status = 'SUCCEEDED' and process_id = " \
                               + str(process_id) + "));"
    ...
    # def dw_data_validation(self, **kwargs):
    #     prev_cnt = []
    #     curr_cnt = []
    #     query_list = []
    #     difference = []
    ...
    #         dag_name = 'zs-DataEngineering-L2-Fact-A1-Reporting-Monthly'
        ...
        try:
            ...
            for index, row in (self.ofr_validation_queries.iterrows()):
                ...
                if row['Category'] == self.msck_repair_tc_key:
                    ...
                    query_curr = row['Queries'].replace('$$db_env',env.lower()).replace('$$pt_cycle_id$$', str(current_cycle_id)).replace(
                        '$$env', env.lower()).replace('$$current_cycle_id$$',str(current_cycle_id)).replace('$$prev_cycle_id$$',str(prev_cycle_id))
    ...
    def adapter_update_run_time(self, process_name, data_date, run_time):
        try:
            print(process_name, data_date, run_time)
            if os.path.exists(self.ec2_excel_path):
                os.remove(self.ec2_excel_path)
            else:
                pass
    ...
    def disease_mapping_qc(self, process, **kwargs):
        try:
            if process == "diseaseQC":
                uploads_path = CommonConstants.UPLOAD_S3_PATH
                code_path = CommonConstants.AIRFLOW_CODE_PATH
                cmd = "aws s3 cp " + uploads_path + "FINAL_DISEASE_MAPPING/disease_mapping.csv " + code_path + "/"

                # CommonUtils().execute_shell_command(cmd)

                command_output = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                                  shell=True)
                ...
                mapping_path = CommonConstants.AIRFLOW_CODE_PATH + "/disease_mapping.csv"
                disease_map = pd.read_csv(mapping_path)
                process_id_list = list(self.qc_validation_queries['Mapping_name'].drop_duplicates())
                query1 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease1"].iloc[0][
                    'Query']
                res1 = ps.sqldf(query1)
                query2 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease2"].iloc[0][
                    'Query']
                res2 = ps.sqldf(query2)
                query3 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease3"].iloc[0][
                    'Query']
                res3 = ps.sqldf(query3)
                query4 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease4"].iloc[0][
                    'Query']
                res4 = ps.sqldf(query4)
                query5 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease5"].iloc[0][
                    'Query']
                res5 = ps.sqldf(query5)
                query6 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease6"].iloc[0][
                    'Query']
                res6 = ps.sqldf(query6)
                query7 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease7"].iloc[0][
                    'Query']
                res7 = ps.sqldf(query7)
                query8 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease8"].iloc[0][
                    'Query']
                res8 = ps.sqldf(query8)
                d1 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease1"].iloc[0][
                    'Description']
                d2 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease2"].iloc[0][
                    'Description']
                d3 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease3"].iloc[0][
                    'Description']
                d4 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease4"].iloc[0][
                    'Description']
                d5 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease5"].iloc[0][
                    'Description']
                d6 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease6"].iloc[0][
                    'Description']
                d7 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease7"].iloc[0][
                    'Description']
                d8 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "Disease8"].iloc[0][
                    'Description']
                msg = disease_mapping_validation_mail(res1, res2, res3, res4, res5, res6, res7, res8, d1, d2, d3, d4,
                                                      d5, d6, d7, d8)
                sender = "zs.clinical@zs.com"
                email_subject = "Disease Mapping validation"
                email_recipient_list = self.configuration.get_configuration(
                    [CommonConstants.ENVIRONMENT_PARAMS_KEY, "delta_email_type_configurations", "ses_recipient_list"])
                status = self.send_email(sender, email_recipient_list, str(msg), email_subject)
                if status:
            ...
            error = "ERROR in reading csv file" + str(e)
            self.execution_context.set_context({"traceback": error})
            logger.error(error, extra=self.execution_context.get_context())

    def ta_mapping_qc(self, process, **kwargs):
        try:
            if process == "taQC":
                uploads_path = CommonConstants.UPLOAD_S3_PATH
                code_path = CommonConstants.AIRFLOW_CODE_PATH
                cmd = "aws s3 cp " + uploads_path + "FINAL_DISEASE_MAPPING/ta_mapping.csv " + code_path + "/"
                # CommonUtils().execute_shell_command(cmd)
                command_output = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE,
                                                  shell=True)
                standard_output, standard_error = command_output.communicate()
                logger.debug(standard_output, extra=execution_context.get_context())
                logger.debug(standard_error, extra=execution_context.get_context())
                mapping_path = CommonConstants.AIRFLOW_CODE_PATH + "/ta_mapping.csv"
                disease_mapping_path = CommonConstants.AIRFLOW_CODE_PATH + "/disease_mapping.csv"
                ta_map = pd.read_csv(mapping_path)
                disease_map = pd.read_csv(disease_mapping_path)
                process_id_list = list(self.qc_validation_queries['Mapping_name'].drop_duplicates())
                query1 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA1"].iloc[0][
                    'Query']
                res1 = ps.sqldf(query1)
                query2 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA2"].iloc[0][
                    'Query']
                res2 = ps.sqldf(query2)
                query3 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA3"].iloc[0][
                    'Query']
                res3 = ps.sqldf(query3)
                query4 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA4"].iloc[0][
                    'Query']
                res4 = ps.sqldf(query4)
                query5 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA5"].iloc[0][
                    'Query']
                res5 = ps.sqldf(query5)
                query6 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA6"].iloc[0][
                    'Query']
                res6 = ps.sqldf(query6)
                query7 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA7"].iloc[0][
                    'Query']
                res7 = ps.sqldf(query7)
                query8 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA8"].iloc[0][
                    'Query']
                res8 = ps.sqldf(query8)
                query9 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA9"].iloc[0][
                    'Query']
                res9 = ps.sqldf(query9)
                query10 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA10"].iloc[0][
                    'Query']
                res10 = ps.sqldf(query10)
                d1 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA1"].iloc[0][
                    'Description']
                d2 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA2"].iloc[0][
                    'Description']
                d3 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA3"].iloc[0][
                    'Description']
                d4 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA4"].iloc[0][
                    'Description']
                d5 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA5"].iloc[0][
                    'Description']
                d6 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA6"].iloc[0][
                    'Description']
                d7 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA7"].iloc[0][
                    'Description']
                d8 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA8"].iloc[0][
                    'Description']
                d9 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA9"].iloc[0][
                    'Description']
                d10 = self.qc_validation_queries[self.qc_validation_queries["Query_id"] == "TA10"].iloc[0][
                    'Description']
                msg = ta_mapping_validation_mail(res1, res2, res3, res4, res5, res6, res7, res8, res9, res10, d1, d2,
                                                 d3, d4, d5, d6, d7, d8, d9, d10)
                sender = "zs.clinical@zs.com"
                email_subject = "TA Mapping validation"
                email_recipient_list = self.configuration.get_configuration(
                    [CommonConstants.ENVIRONMENT_PARAMS_KEY, "delta_email_type_configurations", "ses_recipient_list"])
                status = self.send_email(sender, email_recipient_list, str(msg), email_subject)
                if status:
    ...
    def kpi_ops(self, **kwargs):
        ...
        counts_list2 = []
        try:
            # cmd = "python Glue_Crawler.py -c aws-a0220-use1-00-p-gcr-snfi-ctf-ds-staging"
            ...
            select * from (select distinct 'citeline_trial' as source, pt_batch_id, count(distinct trial_id) as count from snfi_ctfo_datastore_staging_prod.citeline_trialtrove group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'citeline_sites' as source, pt_batch_id, count(distinct site_id) as count from snfi_ctfo_datastore_staging_prod.citeline_organization group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'citeline_investigator' as source, pt_batch_id, count(distinct investigator_id) as count from snfi_ctfo_datastore_staging_prod.citeline_investigator group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'aact_trial' as source, pt_batch_id, count(distinct nct_id) as count from snfi_ctfo_datastore_staging_prod.aact_studies group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'aact_sites' as source, pt_batch_id, count(distinct id) as count from snfi_ctfo_datastore_staging_prod.aact_facilities group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'aact_investigator' as source, pt_batch_id, count(distinct id) as count from snfi_ctfo_datastore_staging_prod.aact_facility_investigators group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'dqs_trial' as source, pt_batch_id, count(distinct member_study_id) as count from snfi_ctfo_datastore_staging_prod.dqs_study group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'dqs_sites' as source, pt_batch_id, count(distinct facility_golden_id) as count from snfi_ctfo_datastore_staging_prod.dqs_study group by 2 order by pt_batch_id desc limit 2)

           ) order by source, pt_batch_id desc"""

            kpi_query2 = """select * from (
                    select * from (select distinct 'dqs_investigator' as source, pt_batch_id, count(distinct person_golden_id) as count from snfi_ctfo_datastore_staging_prod.dqs_study group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'ctms_trial' as source, pt_batch_id, count(distinct study_code) as count from snfi_ctfo_datastore_staging_prod.ctms_study_site group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'ctms_sites' as source, pt_batch_id, count(distinct center_id) as count from snfi_ctfo_datastore_staging_prod.ctms_study_site group by 2 order by pt_batch_id desc limit 2)
            UNION ALL
            select * from (select distinct 'ctms_investigator' as source, pt_batch_id, count(distinct primary_investigator_id) as count from snfi_ctfo_datastore_staging_prod.ctms_study_site group by 2 order by pt_batch_id desc limit 2)
           ) order by source, pt_batch_id desc"""
            ...
            email_recipient_list = ["rakesh.dara@zs.com", "vivek.pangasa@zs.com"]
            sender = "zs.clinical@zs.com"
            email_subject = "zs - Weekly OPS KPIs"
