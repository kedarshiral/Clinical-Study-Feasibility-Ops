...
from Glue_Crawler import glue_crawler
from OFRDataValidationUtility import OFRDataValidationUtility
import Framework_default
import ihmePreIngestion
import diversityPreIngestion
import CopyTAToS3
import tascanCopyTdecToInternal


def tascan_tdec_to_internal():
    tascanCopyTdecToInternal.main()
...
# def call_pre_ingestion():
#     s3 = boto3.client('s3')
#     timestamp = datetime.now().strftime("%Y%m%d")
#     bucket_name = 'aws-a0220-use1-00-d-s3b-shrd-cus-cdl01'
#     file_key = 'clinical-data-lake/uploads/h1_diversity.xlsx'
#     output_file_key = 'clinical-data-lake/pre-ingestion/H1_DIVERSITY/h1_diversity/h1_diversity_{timestamp}.xlsx'.format(timestamp=timestamp)
#
#
...
#     column_rename_map = {
# 		'Patient Race - White / Non-Hispanic': 'percent_white',
# 		'Patient Race - Black / Non-Hispanic': 'percent_african_american',
# 		'Patient Race - Hispanic': 'percent_hispanic',
# 		'Patient Race - Not Identified': 'percent_unknown',
# 		'Patient Race - Asian / Pacific Islander': 'percent_asian_pacific_islander',
# 		'Patient Race - American Indian or Alaska Native': 'percent_american_indian_alaska',
# 		'Patients Age - Under 19': 'patients_age_under_19',
# 		'Patients Age - 19-39' : 'patients_age_19_to_39',
# 		'Patients Age - 40-59' : 'patients_age_40_to_59',
# 		'Patients Age - 60-79' : 'patients_age_60_to_79',
# 		'Patients Age - Over 80' : 'patients_age_over_80',
# 		'Patients Sex - Male' : 'patients_male',
# 		'Patients Sex - Female' : 'patients_female',
# 		'Patients Sex - Other' : 'patients_sex_other',
# 	}
...
#              df_merged.to_excel(writer, index=False, sheet_name='MergedData')
#     s3.put_object(Bucket=bucket_name, Key=output_file_key, Body=output_buffer.getvalue(), ContentType='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
#     print(f'File saved to S3 as {output_file_key}')
#
# def copy_ta_to_s3(file_master_id, process_id, table_name, **kwargs):
...
#     logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh copytatos3.sh "
#                 + str(conf_value) + " " + '"' + str(table_name))
#     command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh copytatos3.sh " \
#                          + '"' + str(conf_value) + '" ' + '"' + str(table_name) + '"'
#     output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
#     logging.info("Output generated by call_delta_calculation task - " + str(output))

def call_pre_ingestion():
    diversityPreIngestion.main()

def call_pre_ingestion_tascan(file_master_id, process_id, **kwargs):
    ...
    query = f"select dataset_name from {audit_db}.ctl_dataset_master where dataset_id = {file_master_id}"
    ...
    dataset_name = query_output[0]['dataset_name']
    file_name = dataset_name.split('tascan_')[1]
    logger.info("================Executing job in Spark client mode================")
    command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh tascanCsvToParquet.sh " + str(file_name)+ " " + str(conf_value)
    logger.info(command_to_execute)
    SystemManagerUtility().execute_command(cluster_id, command_to_execute)

def call_pre_ingestion_ihme():
    ihmePreIngestion.main()
...
def call_validation_utility(dag_id, **kwargs):
    try:
        ...
        if str(dag_id) in ["1000", "2000", "3000"]:
            res = OFRDataValidationUtility().dw_data_validation(dag_id)
        else:
            res = OFRDataValidationUtility().ingestion_validation()
...
def call_standardization(file_master_id, process_id, **kwargs):
    ...
    command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh delta_standardization.sh " \
                         ...
                         + " " + '"' + str(conf_value) + '"'
    output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
    logging.info("Output generated by task - " + str(output))


def call_pre_landing(file_master_id, dag_name, process_id, **kwargs):
    # Framework_default.main(file_master_id,process_id) # new added
...
# def execute_ta(file_master_id, table_name, process_id, **kwargs):
def execute_ta(file_master_id, process_id, table_name, **kwargs):
        """This method calls the delta standardization code"""
        """This method calls dqm"""
        configuration = JsonConfigUtility(
            os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
        ...
        dag_id = str(context['dag_run'].run_id)
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh execute_ta.sh "
                    + str(conf_value) + " " + '"' + str(table_name))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh execute_ta.sh " \
                    + '"' + str(conf_value) + '" ' + '"' + str(table_name) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation task - " + str(output))
...
def call_retention(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "Retention.py")
        status_message = ""
        try:
            status_message = "Started executing shell command " + command
            ...
            standard_output, standard_error = command_output.communicate()
            logger.info(standard_output)
            standard_output_1 = standard_output.decode()
            p = standard_output_1.split('\n')
...
def call_retention_api(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "customize_email_retention.py")
        status_message = ""
        try:
            status_message = "Started executing shell command " + command
            logger.info(status_message)
...
def call_retention_api_inv_prior(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "inv_json_prioritization.py")
        status_message = ""
        try:
            status_message = "Started executing shell command " + command
            logger.info(status_message)
...
def call_retention_api_inv_optimized(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "inv_json_optimized.py")
        status_message = ""
        try:
            status_message = "Started executing shell command " + command
            logger.info(status_message)
...
def call_retention_api_retaining_TU(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "retaining_TU.py")
        status_message = ""
...
def call_retention_api_batch_job(**kwargs):
    try:
        command = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                            "batch_job_retention.py")
        status_message = ""
        try:
            status_message = "Started executing shell command " + command
            logger.info(status_message)
...
def tag_import_export_utility(conf_file_path, **kwargs):
    try:
        ...
        for each in cluster_ids:
            ...
            file_path = "s3:///clinical-data-lake/publish/" \
        ...
        final_site_tag.to_csv(write_path_csv, sep="|", header=False, quotechar='"', doublequote=True, escapechar='"',
                              index=False)
        cmd = "aws s3 cp final_tag.csv s3:///clinical-data-lake/publish/staging/" \
...
def terminate_emr_adhoc():
    ...
    audit_db = configuration.get_configuration([CommonConstants.ENVIRONMENT_PARAMS_KEY, "mysql_db"])
    '''
    query = "SELECT cluster_id FROM " + audit_db + "." + CommonConstants.EMR_CLUSTER_TERMINATE_TABLE + ";"
    logging.info("Executing query - " + query)
    result = MySQLConnectionManager().execute_query_mysql(query, False)
    logger.info("Result of query - " + str(result))
    res_dict=dict(result[0])
    '''

    cluster_ids = ['j-3MXNDBJ8WOWIM']
    for cluster_id in cluster_ids:
        #cluster_id = get_cluster_id(process_id)
        terminate_string = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                     "TerminateEmrHandler.py") + " " + cluster_id
        logging.info("Command to terminate cluster - " + terminate_string)
        execute_shell_command(terminate_string)

def call_delta_calc_std(file_master_id, process_id, type, **kwargs):
    """This method calls the delta standardization code"""
    """This method calls dqm"""
    configuration = JsonConfigUtility(
        os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    ...
    if type == "Generic":
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh delta_calculation.sh "
                    + str(file_master_id) + " " + str(cluster_id) + " " +
                    str(dag_id) + " " + str(process_id) + " " + '"' + str(conf_value))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh delta_calculation.sh " \
                             + str(file_master_id) + " " + str(cluster_id) + " " + str(dag_id) + " " + str(process_id) \
                             + " " + '"' + str(conf_value) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation task - " + str(output))
        ...
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh disease_delta_calc_std.sh "
                    + str(file_master_id) + " " + str(cluster_id) + " " +
                    str(dag_id) + " " + str(process_id) + " " + '"' + str(conf_value))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh disease_delta_calc_std.sh " \
                             + str(file_master_id) + " " + str(cluster_id) + " " + str(dag_id) + " " + str(process_id) \
                             + " " + '"' + str(conf_value) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation_standardization task - " + str(output))

    elif type == "Sponsor":
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh sponsor_delta_calc_std.sh "
                    + str(file_master_id) + " " + str(cluster_id) + " " +
                    str(dag_id) + " " + str(process_id) + " " + '"' + str(conf_value))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh sponsor_delta_calc_std.sh " \
                             + str(file_master_id) + " " + str(cluster_id) + " " + str(dag_id) + " " + str(process_id) \
                             + " " + '"' + str(conf_value) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation_standardization task - " + str(output))
        ...
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh drug_delta_calc_std.sh "
                    + str(file_master_id) + " " + str(cluster_id) + " " +
                    str(dag_id) + " " + str(process_id) + " " + '"' + str(conf_value))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh drug_delta_calc_std.sh " \
                             + str(file_master_id) + " " + str(cluster_id) + " " + str(dag_id) + " " + str(process_id) \
                             + " " + '"' + str(conf_value) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation_standardization task - " + str(output))
        ...
        logger.info("================Executing job in Spark client mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh city_delta_calc_std.sh "
                    + str(file_master_id) + " " + str(cluster_id) + " " +
                    str(dag_id) + " " + str(process_id) + " " + '"' + str(conf_value))
        command_to_execute = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh city_delta_calc_std.sh " \
                             + str(file_master_id) + " " + str(cluster_id) + " " + str(dag_id) + " " + str(process_id) \
                             + " " + '"' + str(conf_value) + '"'
        output = SystemManagerUtility().execute_command(cluster_id, command_to_execute)
        logging.info("Output generated by call_delta_calculation_standardization task - " + str(output))
