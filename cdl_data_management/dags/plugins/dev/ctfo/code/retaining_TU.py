...
def retaining_TU():
    try:
        ...
        for i in df['scenario_id']:
            ...
            insert_trial_universe_query = insert_trial_universe_query.replace("$$foreign_server$$", foreign_server).replace('$$schema_reporting$$', backend_schema).replace('$$schema$$', flask_schema).replace('$$ctfo_trial_ids$$', ctfo_trial_ids)
