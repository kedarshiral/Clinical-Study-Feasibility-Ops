...
class QualtricsUtility():
    ...
    def request_survey_creation(self, parameter_dict, job_id):
        ...
        try:
            ...
            embedded_response = self.res.json()
            if embedded_response["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                return embedded_response
            LOGGING.info("Response after inserting embedded data block for  survey in Qualtrics - %s",
                         str(embedded_response))
            fpi_data = {
                "BackButton": True,
            ...
            back_button_response = self.resp.json()
            if back_button_response["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                return back_button_response
            LOGGING.info("Response after inserting back button data block for  survey in Qualtrics - %s",
                         str(back_button_response))
            return response
    ...
    def invoke_qualtrics_api(self, method_object, parameters_dict, job_id):
        ...
        try:
            ...
            while True:
                if count == 3:
                    break
                LOGGING.info("Attempt - %s", str(count + 1))
                ...
                if response["meta"]["httpStatus"] == CommonServicesConstants.SURVEY_CREATION_STATUS:
                    ...
                    LOGGING.info("error code between 500 to 599 - ")
                count = count + 1
                time.sleep(3)
            return response
        except Exception as e:
    ...
    def create_mailing_list(self, mailing_list_name_dict, job_id):
        ...
        try:
            ...
            if create_mailing_list_response["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
    ...
    def create_qualtrics_contact(self, parameters_dict, job_id):
        ...
        try:
            ...
            print(100 * '&')
            LOGGING.info("parameters dict %s", str(parameters_dict))
            ...
            if create_contact_response_obj["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
        ...
        try:
            ...
            if "result" in response and "nextPage" in response["result"] and response["result"]["nextPage"] is not None:
                ...
                while "result" in resp and "nextPage" in resp["result"] and resp["result"]["nextPage"] is not None:
                    ...
                    resp = resp.json()
                    LOGGING.info("Response from create contact API - {}".format(json.dumps(resp)))

                    if resp["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                        return resp
                    elif "result" in resp and "elements" in resp["result"] and isinstance(resp["result"]["elements"],
                                                                                          list):
                        survey_link_responses["result"]["elements"].extend(resp["result"]["elements"])

            if survey_link_responses["meta"]["httpStatus"] == CommonServicesConstants.SURVEY_CREATION_STATUS:
                return survey_link_responses
            else:
    ...
    def export_responses(self, parameter_dict, job_id):
        ...
        try:
            ...
            for downloadRequestPayload in downloadRequestPayloads:
                print(100 * '&')
                LOGGING.info("downloadRequestPayload %s",str(downloadRequestPayload))
                ...
                downloadRequestResponse = downloadRequestResponse.json()
                if downloadRequestResponse["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                    return downloadRequestResponse
                progressId = downloadRequestResponse["result"]["progressId"]
                ...
                while progressStatus != "complete" and progressStatus != "failed":
                    ...
                    if requestCheckResponse.json()["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
    ...
    def get_survey_metadata(self, parameter_dict, log_params=None):
        ...
        try:
            ...
            if response["meta"]["httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                return response
            LOGGING.info("Final response for Survey Fetched")
            return response
    ...
    def get_survey(self, parameter_dict, log_params=None):
        ...
        """
        try:
            LOGGING.info("Starting function get_survey in Qulatrics ")
    ...
    def get_survey_response(self, parameter_dict, log_params=None):
        ...
        try:
            ...
            elif download_request_response.json()["meta"]["httpStatus"] == CommonServicesConstants.SURVEY_CREATION_STATUS:
            ...
            while progress_status != "complete" and progress_status != "failed":
                LOGGING.info("progressStatus= %s", str(progress_status))
                request_check_url = base_url + progress_id
                request_check_response = requests.request("GET", request_check_url, headers=headers)
                if request_check_response.json()["meta"][
                    "httpStatus"] != CommonServicesConstants.SURVEY_CREATION_STATUS:
                    return request_check_response
    ...
    def get_survey_meta_data_text(self, parameter_dict, log_params=None):
        try:
            LOGGING.debug("Starting function get_survey_metadata in Qulatrics ", extra=log_params)

            # Adding the data centre to fetch the quetion id types for qukatrics

            if 'data_center' in parameter_dict:
                data_center = parameter_dict["data_center"]
            else:
                data_center = self.survey_config_dict["data_center"]

            baseUrl = "https://{data_center}.qualtrics.com/API/v3/survey-definitions/{survey_id}/questions".format(
                data_center=data_center, survey_id=parameter_dict["survey_id"])
            LOGGING.debug("baseUrl - {} ".format(str(baseUrl)), extra=log_params)

            headers = {"x-api-token": parameter_dict["api_token"],
                       "Content-Type": "application/json"}
