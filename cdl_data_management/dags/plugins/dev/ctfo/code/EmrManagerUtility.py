...
def terminate_adhoc_cluster(process_id=None, **kwargs):
    try:
        #Get Cluster_id
        query_str = (
            "select cluster_id from {audit_db}.{EMR_CLUSTER_DETAILS_TABLE} where process_id = '{process_id}' and cluster_status = '{status}'"
        )
        query = query_str.format(
            audit_db=CommonConstants.AUDIT_DB_NAME, status=WAITING_STATE,
            EMR_CLUSTER_DETAILS_TABLE=CommonConstants.EMR_CLUSTER_DETAILS_TABLE,
            process_id=process_id)
        logging.info("Query to get cluster_id from the log table: %s", str(query))
        get_cluster_result = MySQLConnectionManager().execute_query_mysql(query)
        logging.info("Query output: %s", str(get_cluster_result))
        cluster_id = get_cluster_result[0]["cluster_id"]
        logging.info("cluster_id: %s", str(cluster_id))
        region_name = CommonConstants.AWS_REGION
        #Terminate Cluster
        responds = EmrManagerUtility().terminate_cluster(str(cluster_id), region_name)
        logging.info("Output: %s", str(responds))
        # # UPDATE in DB
        query_str = (
            "Update {audit_db}.{EMR_CLUSTER_DETAILS_TABLE} set cluster_status = '{status}' ,"
            " cluster_stop_time = '{end_time}' where cluster_id = '{cluster_id}'"
        )
        update_query = query_str.format(
            audit_db=CommonConstants.AUDIT_DB_NAME, status=TERMINATED_STATE,
            EMR_CLUSTER_DETAILS_TABLE=CommonConstants.EMR_CLUSTER_DETAILS_TABLE,
            end_time=datetime.utcnow(), cluster_id=cluster_id
        )
        logging.info("Query to update the log table: %s", str(update_query))
        response = MySQLConnectionManager().execute_query_mysql(update_query, False)
        logging.info("Query Output: %s", str(response))
    except:
        error_message = "No Cluster ID's are running or fetched. Cluster not terminated."
...
####################################################################################################
    ...
    def __init__(self):
        ...
        APPLICATION_CONFIG_PATH = CommonConstants.AIRFLOW_CODE_PATH + '/' + APPLICATION_CONFIG_FILE
        self.configuration = json.load(open(APPLICATION_CONFIG_PATH))
