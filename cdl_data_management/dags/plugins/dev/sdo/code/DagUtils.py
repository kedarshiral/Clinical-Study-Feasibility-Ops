...
def call_import_export_utility(process_id, conf_file_path, **kwargs):
    try:
        run_command_string = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH,
                                                       "ImportExportUtility.py") + " " + "-f " + os.path.join(
            CommonConstants.AIRFLOW_CODE_PATH, conf_file_path)
        cluster_id = get_cluster_id(process_id)
        #execute_shell_command(run_command_string)
        output = SystemManagerUtility().execute_command(cluster_id, run_command_string)
        logger.info("Output for s3 to RDS step - " + str(output))
...
def call_qc_checks(process_id, **kwargs):
    """This method calls qc check"""
    configuration = JsonConfigUtility(os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    private_key_location = configuration.get_configuration([CommonConstants.ENVIRONMENT_PARAMS_KEY, "private_key_loaction"])
    ip = get_host(process_id, ["MASTER"])
    cluster_id = get_cluster_id(process_id)
    logger.info("IP---------- - " + str(ip))
    # ssh = ExecuteSSHCommand(remote_host=ip, username="hadoop", key_file=private_key_location)
    try:
        logger.info("================Executing job in Spark mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh ")
        # output = ssh.execute_command("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh " )
        command = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh "
        output = SystemManagerUtility().execute_command(cluster_id, command)
        logger.info("Output generated by qc check task - " + str(output))
    except Exception as e:
        print("Couldn't run the QC_Data Validation Module")

def call_citeline_sdo_files_copy(process_id, **kwargs):
    """This method calls function which copies citeline data"""
    configuration = JsonConfigUtility(os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    private_key_location = configuration.get_configuration([CommonConstants.ENVIRONMENT_PARAMS_KEY, "private_key_loaction"])
    ip = get_host(process_id, ["MASTER"])
    cluster_id = get_cluster_id(process_id)
    ...
    try:
        logger.info("================Executing job in Spark mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh citeline_file_copy.sh ")
        # output = ssh.execute_command("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh " )
        command = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh citeline_file_copy.sh "
        output = SystemManagerUtility().execute_command(cluster_id, command)
        logger.info("Output generated by citeline_sdo_files_copy task - " + str(output))
    except Exception as e:
        print("Couldn't run the citeline_sdo_files_copy Module")
...
def call_dqm_filter_ctrl_file(file_master_id, dag_name, process_id, **kwargs):
    """This method calls dqm filter"""
    configuration = JsonConfigUtility(
        os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    private_key_location = configuration.get_configuration(
    ...
    for batch_id in batch_ids:
        if conf_value.find('--deploy-mode cluster') != -1:
            logger.info("================Executing job in Spark cluster mode================")
            conf_value = conf_value + " --name DQMFilter_" + file_master_id + "_" + batch_id
            logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh dqm_filter_ctrl_file.sh " + str(file_master_id) + " " +
                        str(cluster_id) + " " + str(dag_id) + " " + str(process_id) + " " + str(batch_id) + " " + '"' +
                        str(conf_value) + '"' + " " + str(batches))
            try:
                # output = ssh.execute_command("cd " + CommonConstants.AIRFLOW_CODE_PATH +
                ...
                #                              " " + str(batches))
                output = SystemManagerUtility().execute_command(cluster_id, "cd " + CommonConstants.AIRFLOW_CODE_PATH +
                                                                "; sh dqm_filter_ctrl_file.sh " + str(file_master_id) + " " +
                                                                str(cluster_id) + " " + str(dag_id) + " " + str(
                    process_id) +
                                                                " " + str(batch_id) + " " + '"' + str(
                    conf_value) + '"' +
...
def call_mapping_checks(process_id, **kwargs):
    """This method calls mapping checks for study_metadata table"""
    configuration = JsonConfigUtility(os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    private_key_location = configuration.get_configuration([CommonConstants.ENVIRONMENT_PARAMS_KEY, "private_key_loaction"])
    ip = get_host(process_id, ["MASTER"])
    cluster_id = get_cluster_id(process_id)
    ...
    try:
        logger.info("================Executing job in Spark mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh mapping_validation.sh ")
        # output = ssh.execute_command("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh " )
        command = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh mapping_validation.sh "
        output = SystemManagerUtility().execute_command(cluster_id, command)
        logger.info("Output generated by mapping check task for study_metadata table - " + str(output))
    except Exception as e:
        print("Couldn't run the mapping_Data Validation Module for study_metadata files")


def call_filter_protocols(process_id, conf_file_path, **kwargs):
    try:
        run_command_string = "python3 " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "call_filter_protocols.py") + " " + "-f " + os.path.join(CommonConstants.AIRFLOW_CODE_PATH, conf_file_path)
        cluster_id = get_cluster_id(process_id)
        #execute_shell_command(run_command_string)
        output = SystemManagerUtility().execute_command(
            cluster_id, run_command_string
        )
        logger.info("Output for filter protocols - " + str(output))
    except Exception as e:
        logger.error(str(e))
        raise e

def call_ctrl_qc_checks(process_id, **kwargs):
    """This method calls qc check for ctrl table"""
    configuration = JsonConfigUtility(os.path.join(CommonConstants.AIRFLOW_CODE_PATH, CommonConstants.ENVIRONMENT_CONFIG_FILE))
    private_key_location = configuration.get_configuration([CommonConstants.ENVIRONMENT_PARAMS_KEY, "private_key_loaction"])
    ip = get_host(process_id, ["MASTER"])
    cluster_id = get_cluster_id(process_id)
    ...
    try:
        logger.info("================Executing job in Spark mode================")
        logger.info("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation_ctrl.sh ")
        # output = ssh.execute_command("cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation.sh " )
        command = "cd " + CommonConstants.AIRFLOW_CODE_PATH + "; sh qc_data_validation_ctrl.sh "
        output = SystemManagerUtility().execute_command(cluster_id, command)
        logger.info("Output generated by qc check task for ctrl table - " + str(output))
    except Exception as e:
        print("Couldn't run the QC_Data Validation Module for control files")