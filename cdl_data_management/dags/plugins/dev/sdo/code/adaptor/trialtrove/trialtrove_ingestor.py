...
SCHEMA_FILE_NAME = "trialtrove_schema.json"
SQL_FILE_NAME = "trialtrove.sql"
...
class CommonFunctions:
    ...
    def get_bearer_token(self, get_token_url, auth_token_headers, token_payload):
        """
        ...
        try:
            ...
            get_token_response = requests.request(
                method="POST", url=get_token_url, data=token_payload, headers=get_token_headers,
            )
    ...
    def create_input_json_for_spark(
            self, extracted_raw_json_path, application_config,
            batch_id, load_type, s3_deleted_trial_id=None,
    ):
        """
...
class DataIngestionAdapter:
    """
       Purpose             :   This module performs below operation:
                                 a. extract data from pharma_intelligence, ct_gov and pubmed data sources.
                                 b. Perform logging in delta table
       Input               :  a. batch_id
                              b. config_path
                              c. data_source
    ...
    def extract_data_from_pharma_intelligence(
        ...
        try:
            ...
            token_payload = application_config["adapter_details"][self.data_source]["token_pay_load"]
            # secret_password = self.get_secret(application_config["adapter_details"][self.data_source]["token_pay_load"]["password"],
            # application_config["adapter_details"]["generic_config"]["s3_region"])
            # token_payload["password"] = secret_password['password']
            print("***", token_payload["password"])
            token_payload["password"] = token_payload["password"]
            logging.info(token_payload)
            auth_token_headers = application_config["adapter_details"][self.data_source]["token_header"]
            ...
            output_status = self.common_functions.get_bearer_token(
                get_token_url, auth_token_headers, token_payload
            )
            ...
            while True:
                ...
                try:
                    ...
                    status_message = ("For retry '" + str(retry_count) + "' and page_no=" +
                                      str(count) + ", obtained exception " + str(traceback.format_exc())
                                      )
                    logging.debug(str(status_message))
                ...
                if int(response.status_code) == 200:
                    ...
                    self.s3_client.put_object(Body=response.text,
                                              Bucket=bucket_name, Key=key_path, ServerSideEncryption="AES256"
                                              )
                    pre_landing_raw_path = os.path.join(pre_landing_layer,
                                                        datasource + "_" + disease_area_col_value.replace(" ",
                                                                                                          "_") + "_"
                                                        + current_ts + "_" + payload_id + "_" + str(
                                                            self.page_count) + ".json"
                                                        )
        ...
        try:
            ...
            if query_type.lower() == "insert" and table_type.lower() == "smry":
                query = "Insert into {audit_db}.{log_data_acquisition_smry_table} values ('{batch_id}'" \
                        ",'{payload_id}','{data_source}','{payload_json}','{load_type}','{cluster_id}','{status}'" \
                        ",'{start_time}','{end_time}')".format(
                    audit_db=self.audit_db,
                    log_data_acquisition_smry_table=CommonConstants.LOG_DATA_ACQUISITION_SMRY_TABLE,
                    batch_id=batch_id, payload_id=payload_id, load_type=load_type, data_source=self.data_source,
                ...
                query = (
                    ...
                    ",'{payload_id}','{data_source}','{step_name}', '{raw_file_dir}'"
                    ",'{structured_file_path}','{status}','{start_time}','{end_time}')".format(
                        ...
                        batch_id=batch_id, payload_id=payload_id, data_source=self.data_source,
                ...
                query_str = (
                    ...
                    ", end_time = '{end_time}' where trigger_id = '{batch_id}' and "
                ...
                query_str = (
                    ...
                    ", end_time = '{end_time}' where trigger_id = '{batch_id}' and source_name = "
    ...
    def call_create_structured_file(
        ...
        try:
            ...
            command_to_copy_schema_file_to_s3 = (
                    "aws s3 cp " + SCHEMA_FILE_NAME + " " + S3_SCHEMA_LOCATION
            )
            ...
            for dict_data in create_struct_dict_list:
                ...
                spark_input_config["temp_s3_path"] = application_config["adapter_details"]["generic_config"][
                    "s3_temp_path"]
    ...
    def trigger_data_ingestion(
        ...
        try:
            ...
            s3_deleted_trial_id = os.path.join(s3_temp_path_ts, DELETED_TRIAL_ID)
            ...
            output_status = self.common_functions.create_input_json_for_spark(
                extracted_json_path, application_config, batch_id, load_type, s3_deleted_trial_id
            )
    ...
    def get_payload_json(self, payload_id):
        ...
        for dict_data in payload_json["payload_details"]:
            # if not dict_data.get("load_type") or dict_data["load_type"] == "" or type(dict_data["load_type"]!=str):
            #     raise Exception("load_type key/value must be present or must not be empty or must be str")
            # if not dict_data.get("disease_area_col_value") or dict_data["disease_area_col_value"] == "" \
            #         or type(dict_data["disease_area_col_value"]) != str:
            #     raise Exception("disease_area_col_value key/value must be present or must not be empty or must be str")
            #
            # if dict_data["disease_area_col_value"] not in disease_area_col_list and not disease_area_col_list:
            ...
            for payload_json in payload_json_list:
                ...
                if load_type == DELTA_LOAD and not payload_json[0].get("delta_refresh_date_str"):
                    get_max_batch_id_query_str = (
                        "select max(trigger_id) as trigger_id from {audit_db}."
                    ...
                    if not get_max_batch_id_query_output[0]['trigger_id']:
                    ...
                    latest_batch_id = get_max_batch_id_query_output[0]['trigger_id']
                ...
                output_query = self.execute_query(
                    query_type="insert", table_type="smry", batch_id=batch_id, payload_id=payload_id,
                ...
                for sub_payload in payload_json:
                    ...
                    if sub_payload["load_type"] == DELTA_LOAD and not sub_payload.get("delta_refresh_date_str"):
                        logging.info("Delta load is requested but no refresh rate has been given so taking"
                                     " the latest batch as refresh rate")
                        sub_payload["delta_refresh_date_str"] = None
                    if sub_payload["load_type"] == DELTA_LOAD and sub_payload["delta_refresh_date_str"] == None:
                ...
                query_str = (
                    ...
                    "where trigger_id = '{batch_id}' and source_name = '{data_source}' and "
