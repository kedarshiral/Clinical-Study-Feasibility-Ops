...
SCHEMA_FILE_NAME = "organization_schema.json"
SQL_FILE_NAME = "organization.sql"
...
class DataIngestionAdapter:
    ...
    def extract_data_from_pharma_intelligence(
        ...
        try:
            ...
            token_payload = application_config["adapter_details"][self.data_source][
                "token_pay_load"]
            ...
            secret_password = {}
            secret_password['password'] = application_config["adapter_details"][
                self.data_source]["token_pay_load"]["password"]
    ...
    def execute_query(
        ...
        try:
            ...
            if query_type.lower() == "insert" and table_type.lower() == "smry":
                query = "Insert into {audit_db}.{log_data_acquisition_smry_table} " \
                        "values ('{batch_id}'" \
    ...
    def call_create_structured_file(
        ...
        try:
            ...
            ADAPTOR_LOCATION = CommonConstants.EMR_CODE_PATH + "/code/adaptor/organization"
            command_to_copy_schema_file_to_s3 = (
                    "cd " + ADAPTOR_LOCATION + "; aws s3 cp " + SCHEMA_FILE_NAME + " " + S3_SCHEMA_LOCATION
            )
            command_to_copy_sql_file_to_s3 = (
                    "cd " + ADAPTOR_LOCATION + "; aws s3 cp " + SQL_FILE_NAME + " " + S3_SQL_LOCATION
            )
            ...
            for dict_data in create_struct_dict_list:
                ...
                spark_input_config["temp_s3_path"] = application_config["adapter_details"][
                    "generic_config"]["s3_temp_path"]
    ...
    def get_payload_json(self, payload_id):
        ...
        for dict_data in payload_json["payload_details"]:
            # if not dict_data.get("load_type") or dict_data["load_type"] == "" or  type(
            # dict_data["load_type"]!=str):
            #     raise Exception("load_type key/value must be present or must not
            # be empty or must be str")
            ...
            #     raise Exception("disease_area_col_value key/value must be present or must not be
    ...
    def get_adapter_id(self, data_source):
        logging.info("Starting method to get the adapter id")
        ...
        logging.debug("Query to get adapter id - {}".format(get_payload_id_query))
        get_payload_result = MySQLConnectionManager().execute_query_mysql(
            get_payload_id_query
        )
        logging.debug("Query response - {}".format(str(get_payload_result)))
        adapter_id = get_payload_result[0]["adapter_id"]
        adapter_id = str(adapter_id)
        logging.debug(type(adapter_id), adapter_id)
        return adapter_id

    def get_payload_id(self, input_dict, adapter_id):

        logging.info("Starting method to get the payload id")
        ...
        logging.debug("Query to get the payload id - {}".format(check_payload_id_query_str))
        get_payload_result = MySQLConnectionManager().execute_query_mysql(
        ...
        try:
            ...
            for payload_json in payload_json_list:
                ...
                if load_type == DELTA_LOAD and not payload_json[0].get("delta_refresh_date_str"):
                    ...
                    logging.info("Query to insert in the log table: %s", str(query))
                    get_max_batch_id_query_output = \
                        MySQLConnectionManager().execute_query_mysql(query)
                ...
                for sub_payload in payload_json:
                    ...
                    trigger_data_cmd = "cd /usr/local/airflow/dags/plugins/dev/sdo/code/adaptor/common_utilities; python3 " \
                                       "OrganizationAdapter.py ORG " + " " + str(batch_id) + " " + str(payload_id) \
