...
SCHEMA_FILE_NAME = "patientproximityinvestigatorschema.json"
SQL_FILE_NAME = "patientproximityinvestigator.sql"
...
class CommonFunctions:
    ...
    def create_input_json_for_spark(self,
        ...
        try:
            ...
            status_message = ("Input file path for storing converted json= "
                              + converted_json_path
                              )
            logging.debug(str(status_message))
            ...
            if src_s3_flag == "Y":
                ...
                while True:
                    ...
                    if "Contents" in output:
                        for content in output["Contents"]:
                            ...
                            status_message = ("storing converted json input file s3://"
                                              + bucket_name
                                              + "/" + object_path + " to s3://"
                                              + converted_json_bucket_name.rstrip("/")
                                              + "/" + temp_file_path
                                              )
                ...
                for file_name in os.listdir(extracted_raw_json_path):
                    ...
                    status_message = ("storing converted json input file "
                                      + file_name + " to " + target_file_path
                                      )
...
class DataIngestionAdapter:
    ...
    def extract_data_from_pharma_intelligence(
        ...
        try:
            ...
            secret_password = {}
            secret_password['password'] = application_config["adapter_details"][self.data_source][
                "token_pay_load"][
                "password"]
            ...
            full_load_search_url = (application_config["adapter_details"]
                                    [self.data_source]["host_url"]
                                    + application_config["adapter_details"]
                                    [self.data_source]["search_api_url"]
                                    )
            delta_load_url = (application_config["adapter_details"]
                              [self.data_source]["host_url"]
                              + application_config["adapter_details"][self.data_source]
                              ["delta_refresh"]
                              )
            full_load_url = (application_config["adapter_details"][self.data_source]["host_url"]
            ...
            command = ("aws s3 rm --recursive " + pre_landing_layer_batch_id)
            ...
            status_message = ("For URL: '" + url + "' Filter details: "
                              + json.dumps(updated_param_list)
                              )
            logging.debug(str(status_message))
            count = 0
            ...
            while True:
                ...
                file_name = (datasource + "_" + disease_area_col_value.replace(" ", "_")
                             + "_" + current_ts + "_" + payload_id + "_" + str(self.page_count)
                             + ".json")
                ...
                if int(response.status_code) == 200:
                    ...
                    status_message = ("Ingestion " + datasource + " json data of for page '"
                                      + str(count) + "' to " + temp_raw_json_file_path
                                      )
                    logging.debug(str(status_message))
                    ...
                    pre_landing_raw_path = os.path.join(pre_landing_layer,
                                                        ...
                                                        + "_" + current_ts + "_" + payload_id
                                                        + "_" + str(self.page_count) + ".json"
                                                        )

                    command = ("aws  s3 cp " + temp_raw_json_file_path + " " + pre_landing_raw_path)
                    ...
                    status_message = (
                            ...
                            + "' for retry=" + str(retry_count) + " and page_no=" +
                            str(count))
                    ...
                    status_message = "Response from API - {}, authentication token will be " \
                                     "refreshed".format(str(response.status_code))
                    output_status = self.common_functions.get_bearer_token(
                        get_token_url, auth_token_headers, token_payload
                    )
                    if output_status[STATUS_KEY] == FAILED_KEY:
                        return output_status
                    bearer_token = output_status[RESULT_KEY]
                    status_message = "New authentication token was generated successfully"
    ...
    def call_create_structured_file(
        ...
        try:
            ...
            ADAPTOR_LOCATION = CommonConstants.EMR_CODE_PATH + "/code/adaptor/trialtrove"
            command_to_copy_schema_file_to_s3 = (
                    "cd " + ADAPTOR_LOCATION + "; aws s3 cp --debug " + SCHEMA_FILE_NAME + " " + S3_SCHEMA_LOCATION)

            command_to_copy_sql_file_to_s3 = (
                    "cd " + ADAPTOR_LOCATION + "; aws s3 cp --debug " + SQL_FILE_NAME + " " + S3_SQL_LOCATION)
            ...
            output_status = SystemManagerUtility().execute_command(cluster_id, command_to_copy_schema_file_to_s3)
            ...
            output_status = SystemManagerUtility().execute_command(cluster_id, command_to_copy_sql_file_to_s3)
            ...
            try:
                ...
                CustomS3Utility().delete_s3_objects_recursively(os.path.join(S3_SCHEMA_LOCATION, ""))
                ...
                CustomS3Utility().delete_s3_objects_recursively(os.path.join(S3_SQL_LOCATION, ""))
    ...
    def main(self, input_dict):
        ...
        try:
            ...
            logging.info(status_message)
            self.data_source = "patientproximity_investigator"
            ...
            for payload_json in payload_json_list:
                ...
                for sub_payload in payload_json:
                    ...
                    logging.info("sub_payload-----------", sub_payload)
                    print("sub_payload-----------", sub_payload)
                    trigger_data_cmd = "cd /usr/local/airflow/dags/plugins/dev/sdo/code/adaptor/common_utilities; python3 " \
                                       "PatientproximityinvestigatorAdapter.py PINV " + " " + str(batch_id) + " " + str(payload_id) \
                                       + " " + str(FILE_ABS_PATH) + " " + str(cluster_id)
                    ...
                    json_result = json.loads(result_list[0].replace("\'", "\""))
                    ...
                    dict_data["path_extracted_json"] = json_result[SPARK_CONF_PATH]
...
if __name__ == "__main__":
    try:
        ...
        data_source = "patientproximity_investigator"
