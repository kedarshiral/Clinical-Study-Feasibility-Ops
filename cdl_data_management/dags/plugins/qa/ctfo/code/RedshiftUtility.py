...
class RedShiftUtility(object):
    ...
    def execute_redshift_operation(self, input_json):
        ...
        try:
            ...
            if str(self.operation).upper() == 'LOAD':
                process_id = input_json["process_id"]
                rs_cpy_cmd = self.construct_redshift_copy_cmd(process_id)
    ...
    def construct_redshift_copy_cmd(self, process_id):
        ...
        try:
            if process_id == "1300":
                ...
                if self.target_table == "d_rpt_site":
                    print("table is-------->>>>>>>>>", self.target_table)
                    cycle_id = "select max(cycle_id) as curr_cycle_id from " \
                               + audit_db + ".log_cycle_dtl" \
                               + " where cycle_start_time = ( select max(cycle_start_time) from " \
                               + audit_db + ".log_cycle_dtl" \
                               + " where process_id = 2000" + " and cycle_status = 'SUCCEEDED')" \
                               + " and process_id = 2000"
                    data_date = "select data_date as data_date from " \
                                + audit_db + ".log_cycle_dtl" \
                                + " where cycle_start_time = ( select max(cycle_start_time) from " \
                                + audit_db + ".log_cycle_dtl" \
                                + " where process_id = 2000" + " and cycle_status = 'SUCCEEDED')" \
                                + " and process_id = 2000"
                elif self.target_table == "future_bridging_file_site" or self.target_table == "future_bridging_file_inv" or self.target_table == "future_bridging_file_trial" or self.target_table == "past_bridging_file_site":
                    print("table is-------->>>>>>>>>", self.target_table)
                    cycle_id = "select max(cycle_id) as curr_cycle_id from " \
                               + audit_db + ".log_cycle_dtl" \
                               + " where cycle_start_time = ( select max(cycle_start_time) from " \
                               + audit_db + ".log_cycle_dtl" \
                               + " where process_id = 8000" + " and cycle_status = 'SUCCEEDED')" \
                               + " and process_id = 8000"
                    ...
                    cycle_id = "select max(cycle_id) as curr_cycle_id from " \
                               ...
                               + " where cycle_start_time = ( select max(cycle_start_time) from " \
                               + audit_db + ".log_cycle_dtl" \
                               + " where process_id = 3000" + " and cycle_status = 'SUCCEEDED')" \
                               + " and process_id = 3000"
                ...
                sqlquery_cycle_id_result = MySQLConnectionManager().execute_query_mysql(cycle_id, False)
                print("****************************cycle_id*****************")
                print("Query Output for current cycle id", sqlquery_cycle_id_result, cycle_id)
                cycle_id_final = str(sqlquery_cycle_id_result[0]["curr_cycle_id"])
                sqlquery_date_result = MySQLConnectionManager().execute_query_mysql(data_date, False)
                print("Query Output for current data_date", sqlquery_date_result, data_date)
                data_date_1 = str(sqlquery_date_result[0]["data_date"]).replace("-", "")
                print("#####", data_date_1)

                table_complete_name = str(self.schema).__add__(".").__add__(self.target_table)
                file_load_query = "COPY " + str(table_complete_name) + " from '" + str(self.source_s3_location) + "' "
                file_load_query = file_load_query.replace("$$data_dt", data_date_1)
                file_load_query = file_load_query.replace("$$cycle_id", cycle_id_final)
                self.source_s3_location = self.source_s3_location.replace("$$data_dt", data_date_1)
                self.source_s3_location = self.source_s3_location.replace("$$cycle_id", cycle_id_final)
                print("S3 Location is ---->>>>", self.source_s3_location)
    ...
    def _execute_copy_command(self, copy_command, redshift_connection, db_host, db_username, db_password, db_port,
        ...
        try:
            ...
            view_query_1 = "REFRESH MATERIALIZED VIEW ctfo_app_fa_prod.inv_completed_trials_vw"
            view_query_2 = "REFRESH MATERIALIZED VIEW ctfo_app_fa_prod.oncology_ta_vw"
            view_query_3 = "REFRESH MATERIALIZED VIEW ctfo_app_fa_prod.site_workload_vw"


            view_query_result_1 = self._execute_query(view_query_1, redshift_connection, db_host, db_username,
                                                       db_password, db_port, db_name)

            print("View 1 Refreshed -------------->>>>>>>>>>>>>>", view_query_result_1, view_query_1)

            view_query_result_2 = self._execute_query(view_query_2, redshift_connection, db_host, db_username,
                                                       db_password, db_port, db_name)
            print("View 2 Refreshed -------------->>>>>>>>>>>>>>", view_query_result_2, view_query_2)

            view_query_result_3 = self._execute_query(view_query_3, redshift_connection, db_host, db_username,
                                                       db_password, db_port, db_name)
            print("View 3 Refreshed -------------->>>>>>>>>>>>>>", view_query_result_3, view_query_3)
