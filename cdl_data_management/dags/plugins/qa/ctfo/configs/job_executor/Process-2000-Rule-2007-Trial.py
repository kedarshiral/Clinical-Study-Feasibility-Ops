...
from
    ...
    lateral view explode(split(protocol_id, '\\\|')) as protocol_id_1
    group by 1,2) protocol
full outer join
    (select ctfo_trial_id, patient_segment_1 from xref_src_trial_precedence_int
    lateral view explode(split(patient_segment, '\\\#')) as patient_segment_1
    group by 1,2) segment
on protocol.ctfo_trial_id = segment.ctfo_trial_id
...
on final_precedence_trial.ctfo_trial_id = temp.ctfo_trial_id
""")
...
spark.sql("""
INSERT OVERWRITE TABLE sanofi_ctfo_datastore_app_commons_$$db_env.d_trial PARTITION (
        pt_data_dt='$$data_dt',
...
# copy to s3
CommonUtils().copy_hdfs_to_s3('sanofi_ctfo_datastore_app_commons_$$db_env.d_trial')
