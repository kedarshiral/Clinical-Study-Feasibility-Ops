...
#  Pre-requisites      : L1 source table required: sanofi_ctfo_datastore_app_commons_$$db_env.
...
city_standardization.registerTempTable('city_standardization')
...
regexp_replace(get_ascii_value(coalesce(e.standard_city, a.site_city)),"[^0-9A-Za-z, ()_-]","") as site_city,
regexp_replace(get_ascii_value(site_state),"[^0-9A-Za-z, ()_-]","") site_state,
regexp_replace(get_ascii_value(coalesce(d.standard_country, a.site_country, 'Others')),"[^0-9A-Za-z, ()_-]","") as site_country,
site_zip,
d.geo_cd,
...
regexp_replace(trim(lower(d.country)),'[^0-9A-Za-z, ()_-]','')
left outer join city_standardization e
on lower(trim(a.site_city)) = lower(trim(e.city))
""")
...
site_state,
site_country,
scpm.sanofi_cluster as region,
site_zip,
geo_cd,
site_supporting_urls
from d_site_1 a
left join sanofi_cluster_pta_mapping scpm on lower(trim(a.site_country))=lower(trim(scpm.standard_country))
""")
...
spark.sql("""insert overwrite table sanofi_ctfo_datastore_app_commons_$$db_env.d_site partition(
    pt_data_dt='$$data_dt',
    pt_cycle_id='$$cycle_id')
select * from d_site_1
""")
...
# Insert data on S3
CommonUtils().copy_hdfs_to_s3("sanofi_ctfo_datastore_app_commons_$$db_env.d_site")
