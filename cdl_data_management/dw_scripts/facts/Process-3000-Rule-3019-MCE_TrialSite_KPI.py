################################# Module Information #####################################################
#  Module Name         : Metric Engine Site Grain
#  Purpose             : To create the KPIs of Site grain
#  Output              : site_metric_engine_KPI_final
...
trial_site_df = config[(config['Grain'] == 'Trial_Site')]
counter_df = trial_site_df.index
counter = len(counter_df)
trial_site_df = trial_site_df.astype(str)
j = 1
final_query = ""
final_sub_query = ""
KPI_List = []
for index, row in trial_site_df.iterrows():
    if row["AGGREGATE"] == 'N':
...
final_query = final_query.format(on_com=Ongoing_Completed)
print("****loop_query****",final_query)
site_metric_engine_KPI_1 = spark.sql(final_query)
site_metric_engine_KPI_1 = site_metric_engine_KPI_1.dropDuplicates()
site_metric_engine_KPI_1.registerTempTable('site_metric_engine_KPI_1')
#site_metric_engine_KPI_1.write.mode("overwrite").saveAsTable('site_metric_engine_KPI_1')


#############################################
Randomization_rate("Trial_site")
Screen_failure_rate("Trial_site")


site_metric_engine_KPI = spark.sql("""select a.*,b.ctms_sfr as ctms_screen_fail_rate,c.randomization_rate as ctms_randomization_rate,
d.dqs_randomization_rate, d.recruitment_duration AS dqs_total_recruitment_months,
e.duration as ctms_total_recruitment_months
from site_metric_engine_KPI_1 a
left join ctms_screen_fail_rate_site b
on lower(trim(a.ctfo_trial_id))=lower(trim(b.ctfo_trial_id)) and lower(trim(a.ctfo_site_id))=lower(trim(b.ctfo_site_id)) 
left join ctms_randomization_rate_site c
on lower(trim(a.ctfo_trial_id))=lower(trim(c.ctfo_trial_id)) and lower(trim(a.ctfo_site_id))=lower(trim(c.ctfo_site_id))
left join dqs_randomization_rate_site d
on lower(trim(a.ctfo_trial_id))=lower(trim(d.ctfo_trial_id)) and lower(trim(a.ctfo_site_id))=lower(trim(d.ctfo_site_id))
left join ctms_randomization_duration e
on lower(trim(a.ctfo_trial_id))=lower(trim(e.ctfo_trial_id)) and lower(trim(a.ctfo_site_id))=lower(trim(e.ctfo_site_id))
""")
site_metric_engine_KPI = site_metric_engine_KPI.dropDuplicates()
site_metric_engine_KPI.registerTempTable('site_metric_engine_KPI')
#path=path.replace('table_name','site')
#site_metric_engine_KPI.repartition(10).write.mode('overwrite').parquet(path)


#site_metric_engine_KPI.write.mode("overwrite").saveAsTable('site_metric_engine_KPI')
...
trial_site_final_df = config_2[(config_2['Grain'] == 'Trial_Site')]
final_counter_df = trial_site_final_df.index
final_counter = len(final_counter_df)
trial_site_final_df = trial_site_final_df.astype(str)
j = 1
final_coalesce_query = ""
final_coalesce_sub_query = ""
for index, row in trial_site_final_df.iterrows():
...
site_metric_engine_KPI_final = spark.sql(final_coalesce_query)
print("************************ Final Query is executed *************************** ")
site_metric_engine_KPI_final.registerTempTable('site_metric_engine_KPI_final')
site_metric_engine_KPI_final.write.mode("overwrite").saveAsTable('site_metric_engine_KPI_final')
#path_coalesced=path_coalesced.replace('table_name','site')
#site_metric_engine_KPI_final.repartition(10).write.mode('overwrite').parquet(path_coalesced)


spark.sql("""insert overwrite table
$$client_name_ctfo_datastore_app_fa_$$db_env.mce_trial_site_kpi partition(pt_data_dt, pt_cycle_id)
select *, "$$data_dt" as pt_data_dt,
   "$$cycle_id" as pt_cycle_id
from
   site_metric_engine_KPI_final
""")

if site_metric_engine_KPI_final.count() == 0:
    print("Skipping copy_hdfs_to_s3 for mce_trial_site_kpi as zero records are present.")
else:
    CommonUtils().copy_hdfs_to_s3("$$client_name_ctfo_datastore_app_fa_$$db_env.mce_trial_site_kpi")
