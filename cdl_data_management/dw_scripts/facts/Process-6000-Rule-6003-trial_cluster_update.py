...
union
select *, null as new_golden_id from Trial_records where lower(trim(type)) = 'd' """)
...
protocol_id, mesh_terms, trial_phase, a.type,
...
inner join xref_src_trial_precedence_int b on trim(a.exp_golden_id) = trim(b.ctfo_trial_id)
order by 
...
csv_path = bucket_path + '/applications/commons/temp/km_validation/MSD_cluster_update/archive/output/trial/pt_data_dt=$$data_dt/pt_cycle_id=$$cycle_id'
