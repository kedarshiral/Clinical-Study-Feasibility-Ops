...
#  Pre-requisites      : Source table required: temp_all_site_data_prep_aact_clustering
...
cur.execute('drop table if exists temp_all_site_match_score_aact')
cur.execute('create table temp_all_site_match_score_aact (cluster_id string, uid bigint, score double) '
            'stored as parquet')
...
country_list_query = """select distinct country from temp_all_site_data_prep_aact_clustering"""
cur = hive.connect(host='localhost', port=10000).cursor()
...
def execute_dedupe_country_ta(country_ta, output_list):
    ...
    country_ta_data = """select uid, name, city, country, case when zip like '0%' then
     REGEXP_REPLACE(zip, '^0+', '') else zip end as zip, address  from 
       temp_all_site_data_prep_aact_clustering where name is not null and lower(country) = '""" + country_ta + \
                      """'"""
    ...
    try:
        ...
        # print duplicates
        query = 'insert into temp_all_site_match_score_aact values '
...
def execute_dedupe(country, output_list):
    ...
    try:
        country_ta_list_query = """select distinct country as country_ta from temp_all_site_data_prep_aact_clustering where lower(trim(country)) = '""" + country + """'"""
        cur = hive.connect(host='localhost', port=10000).cursor()
