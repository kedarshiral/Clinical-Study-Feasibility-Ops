#################################### Module Information ######################################
#  Module Name         : All Investigator Precedence
#  Purpose             : This will execute queries to apply precedence rule on data coming
#                                                from xref_src_inv and to prepare data for final_precedence_investigator
#                                                table
#  Pre-requisites      : Source table required: xref_src_inv
#  Last changed on     : 22-03-2021
#  Last changed by     : Himanshi
#  Reason for change   : Added score precedence
#  Return Values       : NA
############################################################################################
################################### High level Process #####################################
# Prepare data for final_precedence_investigator table
############################################################################################
import json
import pyspark
...
spark = pyspark.sql.SparkSession.builder.appName(
    'AllInvPrecedence').enableHiveSupport().getOrCreate()
...
data_source_name_column = config_data["modules"]["investigators_dedupe"]["data_source_name_column"]
source_id_column_name = config_data["modules"]["investigators_dedupe"]["source_id_column_name"]
score_column_name = config_data["modules"]["investigators_dedupe"]["score_column_name"]
# Reading XREF Table
xref_table_name = config_data["modules"]["investigators_dedupe"]["xref_table_name"]
...
# Sorting table based on given data source, score and UID (which is made after sorting the cols. to make it consistent across cycles)
query_for_sorting = ""
count = 1
for elem in sorted_data_source_order_dict:
    query_for_sorting = query_for_sorting + """ When {col_name} == '{data_source_name}' THEN	
    {order_no}""".format(
        col_name=data_source_name_column, data_source_name=elem[0], order_no=count)
    count += 1
query_for_sorting = """SELECT *, CASE""" + query_for_sorting + """ END AS rank_data_src from temp order by rank_data_src asc,{score_column_name} desc,{source_id_column_name} asc """.format(score_column_name =score_column_name,source_id_column_name=source_id_column_name)
...
golden_id_column = config_data["modules"]["investigators_dedupe"]["golden_id"]
...
for i in range(len(columns_list)):
	if i == len(columns_list) - 1:
		query_for_partitioning = query_for_partitioning + """ collect_list(""" + columns_list[i] + """) over (PARTITION BY {golden_id} ORDER BY rank_data_src asc,  {score_column_name} desc,{source_id_column_name} asc) as""".format(golden_id=golden_id_column,score_column_name=score_column_name, source_id_column_name=source_id_column_name) + """ """ + columns_list[i] + """_list"""
	else:
		query_for_partitioning = query_for_partitioning + """ collect_list(""" + columns_list[i] + """) over (PARTITION BY {golden_id} ORDER BY rank_data_src asc, {score_column_name} desc,{source_id_column_name} asc ) as""".format(golden_id=golden_id_column,score_column_name=score_column_name, source_id_column_name=source_id_column_name) + """ """ + columns_list[i] + """_list,"""
...
#partitioned_1_df.write.mode("overwrite").saveAsTable("partitioned_1_df_inv")
# Preparing a temporary table for final precedence table. This is an additional
#  transformation required for final_precedence.
...
# Creating final precedence table
...
final_precedence_df_new = spark.sql(final_precedence_query)
final_precedence_df_new = final_precedence_df_new.drop("pt_data_dt").drop("pt_cycle_id").drop("src_investigator_id").drop("hash_uid")
# Finally writing to hive.
final_precedence_df_new.write.mode('overwrite').saveAsTable('final_precedence_investigator')