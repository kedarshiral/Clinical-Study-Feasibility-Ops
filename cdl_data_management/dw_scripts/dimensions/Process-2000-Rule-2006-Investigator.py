...
select /*+ broadcast(d) */
 ...
 initcap(regexp_replace(regexp_replace(get_ascii_value(investigator_name),"[^0-9A-Za-z, ()_-]",""), " +", " ")) as investigator_name
,investigator_phone
,investigator_email
,investigator_specialty
,regexp_replace(regexp_replace(get_ascii_value(investigator_city),"[^0-9A-Za-z, ()_-]",""), " +", " ") as investigator_city
,regexp_replace(regexp_replace(get_ascii_value(investigator_state),"[^0-9A-Za-z, ()_-]",""), " +", " ") as investigator_state
,investigator_zip
,regexp_replace(regexp_replace(get_ascii_value(coalesce(d.standard_country,investigator_country, 'Other') ),"[^0-9A-Za-z, ()_-]",""), " +", " ") as investigator_country
,regexp_replace(regexp_replace(get_ascii_value(investigator_address),"[^0-9A-Za-z, ()_-]",""), " +", " ") as investigator_address
,d.geo_cd,
...
temp_d_investigator_1.write.mode('overwrite').saveAsTable('d_investigator')

# Insert data in hdfs table
spark.sql("""insert overwrite table $$client_name_ctfo_datastore_app_commons_$$db_env.d_investigator
...
# Insert data on S3
CommonUtils().copy_hdfs_to_s3("$$client_name_ctfo_datastore_app_commons_$$db_env.d_investigator")