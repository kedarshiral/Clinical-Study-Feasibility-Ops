...
# Reading config file.
json_file_path = CommonConstants.EMR_CODE_PATH+'/configs/job_executor/precedence_params.json'
json_file = open(json_file_path, 'r').read()
...
rank_data_src asc, {score_column_name} desc,{source_id_column_name} asc""".format(score_column_name =score_column_name,source_id_column_name=source_id_column_name)
...
temp_df_sort_by_data_src_nm_1 = spark.sql("""select temp_df_sort_by_data_src_nm.*,
case when lower(trim(trial_status)) like '%ongoing%' then 1
when lower(trim(trial_status)) like '%completed%' then 2
when lower(trim(trial_status)) like '%planned%' then 3
when lower(trim(trial_status)) like '%others%' then 4
else 5 end as status_precedence
from temp_df_sort_by_data_src_nm """)
temp_df_sort_by_data_src_nm_1.createOrReplaceTempView('temp_df_sort_by_data_src_nm_1')
#temp_df_sort_by_data_src_nm_1.write.mode("overwrite").saveAsTable("temp_df_sort_by_data_src_nm_1")
...
for i in range(len(columns_list)):
    if i == len(columns_list) - 1:
        query_for_partitioning = query_for_partitioning + """ collect_list(""" + columns_list[i] + """) over (PARTITION BY {golden_id} ORDER BY rank_data_src asc,status_precedence asc ,{score_column_name} desc,  {source_id_column_name} asc) as""".format(golden_id=golden_id_column,score_column_name=score_column_name, source_id_column_name=source_id_column_name) + """ """ + columns_list[i] + """_list"""
    else:
        query_for_partitioning = query_for_partitioning + """ collect_list(""" + columns_list[i] + """) over (PARTITION BY {golden_id} ORDER BY rank_data_src asc,status_precedence asc, {score_column_name} desc, {source_id_column_name} asc) as""".format(golden_id=golden_id_column,score_column_name=score_column_name, source_id_column_name=source_id_column_name) + """ """ + columns_list[i] + """_list,"""
query_for_partitioning = query_for_partitioning + """ FROM temp_df_sort_by_data_src_nm_1 """
...
#partitioned_1_df.write.mode("overwrite").saveAsTable("partitioned_1")
...
#temp_table_1_df.write.mode("overwrite").saveAsTable("temp_table_1")
...
write_path = path.replace('table_name', 'final_precedence_df')
final_precedence_df.repartition(100).write.mode('overwrite').parquet(write_path)