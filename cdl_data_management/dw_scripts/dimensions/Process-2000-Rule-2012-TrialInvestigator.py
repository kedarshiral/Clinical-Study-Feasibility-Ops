...
select
        ...
        'aact' as data_source
from $$client_name_ctfo_datastore_staging_$$db_env.aact_facility_investigators
group by 1,2,3
""")
...
select
    investigator_id as src_inv_id,
        new_past_trials_id as src_trial_id,
        'citeline' as data_source
from $$client_name_ctfo_datastore_staging_$$db_env.citeline_investigator
lateral view outer explode(split(past_trials_id,'\\\|'))one as new_past_trials_id
union
select
    investigator_id as src_inv_id,
new_ongoing_trials_id as src_trial_id,
        'citeline' as data_source
from $$client_name_ctfo_datastore_staging_$$db_env.citeline_investigator
lateral view outer explode(split(ongoing_trials_id,'\\\|'))one as new_ongoing_trials_id
""")

citeline_trial_inv.registerTempTable('citeline_trial_inv')

tascan_trial_inv = spark.sql("""
select
    investigator_id as src_inv_id,
    ongoing_trial_id as src_trial_id,
    'tascan' as data_source
from $$client_name_ctfo_datastore_app_commons_$$db_env.tascan_investigator
lateral view outer explode(split(ongoing_trial_ids,'\\\|'))one as ongoing_trial_id
""")

tascan_trial_inv.registerTempTable('tascan_trial_inv')
...
select
    src_investigator_id as src_inv_id,
    src_trial_id,
...
select
        *
from tascan_trial_inv
""")

all_trial_inv.registerTempTable('all_trial_inv')


trial_ta_2012 = spark.sql(""" select src_trial_id, ctfo_trial_id, data_src_nm from
$$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_trial
""")
trial_ta_2012.registerTempTable('trial_ta_2012')
...
trial_ta_2012 xref_src_trial group by 1,2,3)
 xref_src_trial
...
inner join
    ...
    where xref_src_inv.ctfo_investigator_id is not null and xref_src_trial.ctfo_trial_id is not null
    group by 1,2
""")
...
$$client_name_ctfo_datastore_app_commons_$$db_env.r_trial_investigator
...
# Insert data on S3
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.r_trial_investigator')