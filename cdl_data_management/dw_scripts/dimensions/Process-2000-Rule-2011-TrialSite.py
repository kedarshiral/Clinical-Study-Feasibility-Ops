...
select
        ...
        'aact' as data_source
from
$$client_name_ctfo_datastore_staging_$$db_env.aact_facilities
group by 1,2
""")
aact_trial_site.registerTempTable('aact_trial_site')

tascan_trial_site = spark.sql("""
select
        trim(site_id) as src_site_id,
        trim(site_trial_id) as src_trial_id,
        'tascan' as data_source
from
$$client_name_ctfo_datastore_app_commons_$$db_env.tascan_site
group by 1,2
""")
tascan_trial_site.registerTempTable('tascan_trial_site')
...
from
$$client_name_ctfo_datastore_app_commons_$$db_env.usl_study_site_investigator
group by 1,2,3
""")
...
from
$$client_name_ctfo_datastore_staging_$$db_env.citeline_organization
lateral view outer explode(split(site_trial_id,'\\\;')) as src_trial_id
group by 1,2
""")
...
select
        ...
        'ir' as data_source
from
$$client_name_ctfo_datastore_app_commons_$$db_env.usl_ir
group by 1,2
""")
...
select
        *
from tascan_trial_site
""")

all_site_trial.registerTempTable('all_site_trial')

#trial_ta_temp = spark.sql(""" select src_trial_id, ctfo_trial_id, data_src_nm from
#$$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_trial
#lateral view outer explode(split(xref_src_trial.therapeutic_area,'\\\|')) as trial_therapeutic_area""")
#trial_ta_temp.registerTempTable('trial_ta_temp')
...
$$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_trial  group by 1,2,3) xref_src_trial
...
$$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_site group by 1,2,3) xref_src_site
    ...
    where xref_src_site.ctfo_site_id is not null and xref_src_trial.ctfo_trial_id is not null 
    group by 1,2
""")
r_trial_site.registerTempTable('r_trial_site')

# Insert data in hdfs table
spark.sql("""insert overwrite table $$client_name_ctfo_datastore_app_commons_$$db_env.r_trial_site
...
# Push data on S3
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.r_trial_site')