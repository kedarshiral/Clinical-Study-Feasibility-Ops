...
##Reading past bridging file to update the run_latest column based on the current run
if (CommonConstants.BRIDGING_RUN_FIRST == 'N'):
    cursor.execute(
    ...
    path = "{}/applications/commons/dimensions/past_bridging_file/pt_data_dt={}/pt_cycle_id={}/".format(
    bucket_path, latest_stable_data_date, latest_stable_cycle_id)
else :
    path = "{}/uploads/past_bridging_file/output_tables/main_table_with_tag/".format(
    bucket_path)

past_bridging_file = spark.read.parquet(path)
past_bridging_file.registerTempTable('past_bridging_file')
...
if (CommonConstants.BRIDGING_RUN_FIRST == 'Y'):
    ...
    final_df = spark.sql(
        """ select distinct case when lower(trim(data_src_nm)) in ('aact','tascan') then concat(data_src_nm,'_',hash_uid) else concat(data_src_nm,'_',src_site_id)  end as key, ctfo_site_id as run_latest,ctfo_site_id as run_1 from $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_site_precedence_int """)
    ...
    latest_stable_data_date = str(fetch_enable_flag_result['data_date']).replace("-", "")
    latest_stable_cycle_id = str(fetch_enable_flag_result['cycle_id'])
    path = "{}/applications/commons/dimensions/future_bridging_file_site/pt_data_dt={}/pt_cycle_id={}/".format(
        bucket_path, latest_stable_data_date, latest_stable_cycle_id)
    ...
    current_data = spark.sql(
        """ select distinct case when lower(trim(data_src_nm)) in ('aact','tascan') then concat(data_src_nm,'_',hash_uid) else concat(data_src_nm,'_',src_site_id)  end as key, ctfo_site_id as run_latest,ctfo_site_id as run_1  from $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_site_precedence_int group by 1,2""")
...
spark.sql("""
   ...
   $$client_name_ctfo_datastore_app_commons_$$db_env.future_bridging_file_site PARTITION (
           pt_data_dt='$$data_dt',
   ...
   FROM final_df
   """)
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.future_bridging_file_site')

##Update the past bridging file to populate golden ids corresponding to this run
Columns = past_bridging_file.columns
Columns.remove('key')
Columns.remove('key_without_ta')
Columns.remove('run_latest')
rest_columns = ''
for i in range(0, len(Columns)):
    rest_columns = rest_columns + 'a.' + Columns[i] + ','
rest_columns = rest_columns[0:len(rest_columns) - 1]
query = " select distinct a.key_without_ta, a.key,b.run_latest as run_latest,{rest_columns} from past_bridging_file a left join final_df b on lower(trim(a.key_without_ta ))=lower(trim(b.key)) ".format(
    rest_columns=rest_columns)
final_past_df = spark.sql(query)
final_past_df.registerTempTable('final_past_df')

spark.sql("""
   INSERT OVERWRITE TABLE
   $$client_name_ctfo_datastore_app_commons_$$db_env.past_bridging_file PARTITION (
           pt_data_dt='$$data_dt',
           pt_cycle_id='$$cycle_id'
           )
   SELECT *
   FROM final_past_df
   """)
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.past_bridging_file')
