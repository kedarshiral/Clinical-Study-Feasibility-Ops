...
spark = pyspark.sql.SparkSession.builder.appName(
    'AllSitePrecedence').enableHiveSupport().getOrCreate()
...
data_source_name_column = config_data["modules"]["sites_dedupe"]["data_source_name_column"]
source_id_column_name = config_data["modules"]["sites_dedupe"]["source_id_column_name"]
score_column_name = config_data["modules"]["sites_dedupe"]["score_column_name"]


# Reading XREF Table
xref_table_name = config_data["modules"]["sites_dedupe"]["xref_table_name"]
...
for elem in sorted_data_source_order_dict:
    ...
    count += 1
query_for_sorting = """SELECT *, CASE""" + query_for_sorting + """ END AS rank_data_src from temp order by
rank_data_src asc,{score_column_name} desc,{source_id_column_name} asc""".\
    format(score_column_name=score_column_name, source_id_column_name=source_id_column_name)
...
#temp_df_sort_by_data_src_nm.write.mode('overwrite').saveAsTable('temp_df_sort_by_data_src_nm')


# Getting golden_id_column
golden_id_column = config_data["modules"]["sites_dedupe"]["golden_id"]
...
# Perform collect_list over each window partition to get the desired list for all required
# columns.  Reason to do is to maintain ordering for collect_list.
...
#partitioned_1_df.write.mode('overwrite').saveAsTable('partitioned_1_df_site')


# Preparing a temporary table for final precedence table.  This is an additional transformation
# required for final_precedence.
...
### dropping extra cols. which are not required
final_precedence_df = final_precedence_df.drop("pt_data_dt").drop("pt_cycle_id").drop("src_site_id").drop("hash_uid")

# Finally writing to hive.
final_precedence_df.write.mode('overwrite').saveAsTable('final_precedence_site')
