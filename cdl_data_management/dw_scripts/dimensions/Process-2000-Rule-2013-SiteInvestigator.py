...
select
        trim(aact_site.src_site_id) as src_site_id,
        trim(inv.src_inv_id) as src_inv_id,
        'aact' as data_source
from
(select trim(id) as src_site_id from $$client_name_ctfo_datastore_staging_$$db_env.aact_facilities
 group by 1) aact_site
left outer join
(select facility_id,trim(id) as src_inv_id  from
$$client_name_ctfo_datastore_staging_$$db_env.aact_facility_investigators group by 1,2) inv
on lower(trim(aact_site.src_site_id)) = lower(trim(inv.facility_id))
group by 1,2,3
""")
aact_site_inv.registerTempTable('aact_site_inv')

# Fetching Citeline site - investigator mapping data
...
from
$$client_name_ctfo_datastore_staging_$$db_env.citeline_organization lateral view outer explode
(split(investigator_id,"\;"))one as src_inv_id
""")
citeline_site_inv.registerTempTable('citeline_site_inv')

tascan_site_inv = spark.sql("""
select
        trim(investigator_site_id) as src_site_id,
        trim(investigator_id) as src_inv_id,
        'tascan' as data_source
from
$$client_name_ctfo_datastore_app_commons_$$db_env.tascan_investigator
""")

tascan_site_inv.registerTempTable('tascan_site_inv')
...
# Fetching CTMS site data
ctms_site_inv = spark.sql("""
select
        trim(src_site_id) as src_site_id,
        trim(src_investigator_id) as src_inv_id,
        source as data_source
from $$client_name_ctfo_datastore_app_commons_$$db_env.usl_study_site_investigator group by 1,2,3
""")
...
inner join
    ...
    $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_site group by 1,2,3) xref_src_site
...
inner join
    (select src_investigator_id, ctfo_investigator_id, data_src_nm from
     $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_inv xref_src_inv group by 1,2,3)
     xref_src_inv
    ...
    where
     xref_src_inv.ctfo_investigator_id is not null and xref_src_site.ctfo_site_id is not null 
    group by 1,2
""")
r_site_investigator.registerTempTable('r_site_investigator')

# Insert data in hdfs table
spark.sql("""insert overwrite table $$client_name_ctfo_datastore_app_commons_$$db_env.r_site_investigator
...
# Insert data on S3
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.r_site_investigator')