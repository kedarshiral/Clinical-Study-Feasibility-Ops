...
if (CommonConstants.BRIDGING_RUN_FIRST == 'Y'):
    ...
    final_df = spark.sql(
        """ select distinct concat(data_src_nm,'_',src_trial_id)  as key, ctfo_trial_id as run_latest,ctfo_trial_id as run_1 from $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_trial_precedence_int """)
    ...
    path = "{}/applications/commons/dimensions/future_bridging_file_trial/pt_data_dt={}/pt_cycle_id={}/".format(
    ...
    current_data = spark.sql(
        """ select distinct concat(data_src_nm,'_',src_trial_id)  as key, ctfo_trial_id as run_latest,ctfo_trial_id as run_1 from $$client_name_ctfo_datastore_app_commons_$$db_env.xref_src_trial_precedence_int group by 1,2""")
...
spark.sql("""
   ...
   $$client_name_ctfo_datastore_app_commons_$$db_env.future_bridging_file_trial PARTITION (
           pt_data_dt='$$data_dt',
   ...
   FROM final_df
   """)
CommonUtils().copy_hdfs_to_s3('$$client_name_ctfo_datastore_app_commons_$$db_env.future_bridging_file_trial')
