...
MODULE_NAME = "DiseaseStandardizationProposed.py"


class DiseaseStandardizationProposed(object):
    def __init__(self):
        print("init done")
    ...
    def standardize_aact(self, disease_df):
        ...
        aact_df["processed"] = aact_df["processed"].apply(
            lambda x: self.removeStopwords(x)
        )

        df = aact_df.copy()

        df_clean = pd.read_excel("std_syn_one.xlsx")
        ...
        tf_idf_matrix_clean = vectorizer.fit_transform(df_clean["std_disease"].values.astype('U'))
        ...
        try:
            matches = self.cossim_top(
                tf_idf_matrix_dirty, tf_idf_matrix_clean.transpose(), 1, 0
            )
        ...
        matches_df = self.get_matches_df(
            matches, df["processed"], df_clean["std_disease"]
        )
        ...
        df1 = pd.read_excel("exploded.xlsx")
        std_disease_list = list(set(df1["standard_disease"].tolist()))
        df1["disease"] = df1["disease"].apply(
            str
        )  # to convert NaN (nan) values to str to avoid type error in process extract 2049 nan values
        df1.drop_duplicates(
            subset=["disease"], keep="first", inplace=True
        )  # remove duplicates from synonyms column for below line of code to work (unique values required)
        matches_df["Standard Names"] = (
            matches_df["std_syn"]
                .map(df1.set_index("disease")["standard_disease"])
                .fillna(matches_df["std_syn"])
        )
        ...
        aact_df.to_excel("aact_diseases.xlsx", index=False)  # choose 0.41 as threshold
    ...
    def standardize_dqs(self, disease_df):
        ...
        dqs_df["processed"] = dqs_df["processed"].apply(
            lambda x: self.removeStopwords(x)
        )
        ...
        dqs_df.to_excel("dqs_diseases.xlsx", index=False)
    ...
    def standardize_citeline(self, disease_df):
        ...
        try:
            ...
            ctln_df["processed"] = ctln_df["processed"].apply(
                lambda x: self.removeSpecial(x)
            )
        ...
        citeline_df.to_excel("citeline_diseases.xlsx", index=False)
    ...
    def standardize_pharma_diseases(self, disease_df):
        ...
        pharma_df["processed"] = pharma_df["processed"].apply(
            lambda x: self.removeSpecial(x)
        )
        ...
        pharmadrug_disease_df.to_excel("pharmadrug_diseases.xlsx", index=False)
    ...
    def standardize_drg_diseases(self, disease_df):
        ...
        drg_df["processed"] = drg_df["processed"].apply(
            lambda x: self.removeStopwords(x)
        )
        ...
        drg_disease_df.to_excel("drg_diseases.xlsx", index=False)
    ...
    def standardize_data_monitor_diseases(self, disease_df):
        ...
        dmon_df["processed"] = dmon_df["processed"].apply(
            lambda x: self.removeSpecial(x)
        )
        ...
        dmon_disease_df.to_excel("dmon_diseases.xlsx", index=False)
    ...
    def standardize_globocan_diseases(self, disease_df):
        print("Started mapping for globocan diseases data...")

        print("disease df:", disease_df[disease_df["datasource"] == "globocan"].shape[0])

        disease_df.rename(columns={"disease_name": "disease"}, inplace=True)

        globocan_df = disease_df[disease_df["datasource"] == "globocan"].copy()
        ...
        globocan_df["processed"] = globocan_df["processed"].apply(
            lambda x: x + " cancer"
        )
        ...
        globocan_disease_df.to_excel("globocan_diseases.xlsx", index=False)
    ...
    # added
    def standardize_ctms_diseases(self, disease_df):
        ...
        ctms_df = disease_df[disease_df["datasource"] == "ctms"].copy()

        ctms_df["disease"] = ctms_df["disease"].str.strip()
        ctms_df.drop_duplicates(subset="disease", inplace=True)

        # ctms_df.dropna(inplace=True)
        print("Number of records in globocan: ", ctms_df.shape[0])

        ctms_df["processed"] = ctms_df["disease"].apply(str)

        ctms_df["processed"] = ctms_df["processed"].apply(lambda x: x + " cancer")

        ctms_df["processed"] = ctms_df["processed"].apply(
            lambda x: self.removeSpecial(x)
        )
        ...
        ctms_disease_df = test.merge(df, on="processed", how="inner")

        print("Completed standardization of ctms diseases")
        print("Writing output to CSV File..")

        ctms_disease_df.to_excel("ctms_diseases.xlsx", index=False)

        print("Done!!")

        return ctms_disease_df
    ...
    def main(self):
        ...
        exploded_file = os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "/exploded.xlsx")
        synonym_file = os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "/std_syn_one.xlsx")
        delta_file = os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "/delta.xlsx")

        if os.path.exists(exploded_file):
            os.system("rm " + exploded_file)
        exploded_file_df=pd.read_excel("disease_mapping_parexcel.xlsx")
        exploded_file=exploded_file_df[['standard_disease', 'disease']].drop_duplicates()
        exploded_file.to_excel("exploded.xlsx",index=False)
        #os.system(
        #    "aws s3 cp s3://{}/clinical-data-lake/uploads/EXPLODED_FILE/exploded.csv ./".format(
        #        s3_bucket_name
        #    )
        #)
        ...
        std_syn_one_df=pd.read_excel("disease_mapping_parexcel.xlsx")
        std_syn_one=std_syn_one_df[['standard_disease']].drop_duplicates()
        std_syn_one.rename({'standard_disease':'std_disease'},inplace=True,axis=1)
        std_syn_one.to_excel("std_syn_one.xlsx",index=False)
        #os.system(
        #    "aws s3 cp s3://{}/clinical-data-lake/uploads/SYNONYM_FILE/std_syn_one.csv ./".format(
        #        s3_bucket_name
        #    )
        #)
        ...
        os.system("rm -r" + CommonConstants.AIRFLOW_CODE_PATH + "/Delta")
        os.system(
            "hadoop fs -copyToLocal /user/hive/warehouse/Delta "
            + CommonConstants.AIRFLOW_CODE_PATH
            + "/"
        )
        os.system(
        ...
        nltk.download("punkt")
        #delta_xlsx=pd.read_csv("delta.csv", encoding="ISO-8859-1")
        #delta_xlsx.to_excel("delta.xlsx",index=False)
        # Reading File
        disease_df = pd.read_excel("delta.xlsx")
        ...
        # added
        if "ctms" in df_list:
            # used same function for ctms as well as globocan
            ctms_df = self.standardize_ctms_diseases(disease_df)

            if len(ctms_df) > 0:
                ctms_df.drop(columns=["std_syn", "index"], inplace=True)
                df_to_combine.append(ctms_df)

        if "globocan" in df_list:
            globocan_df = self.standardize_globocan_diseases(disease_df)
        ...
        temp_df = pd.DataFrame(
            columns=["unique_id", "trial_id", "datasource", "disease"]
        )
        ...
        final_data.rename(
            columns={
                "synonym": "Synonyms",
                "Standard_Names": "proposed_mesh",
                "disease_name": "mesh_name",
            },
            inplace=True,
        )

        final_data.to_excel("delt_mesh_mapping_temp.xlsx", index=False)

        # copying existing disease mapping file from s3 to current path i.e code folder
        '''
        os.system(
            "aws s3 cp s3://{}/clinical-data-lake/uploads/FINAL_DISEASE_MAPPING/disease_mapping.csv ./".format(
                s3_bucket_name
            )
        )
        ...
        merge_new_to_s3.to_excel("delt_disease_mapping_temp.xlsx", index=False)
        merge_new_to_s3.drop_duplicates(
            subset=["Synonyms", "standard_disease"], inplace=True
        )
        '''
        # final_data[
        #   ['unique_id', 'datasource', 'disease_name', 'processed', 'Standard_Names', 'synonym',
        #   'similarity', 'trial_id']].to_excel("final_disease_mapping_11092020.csv", index=False)
        print(
            "Output has been stored in xlsx file disease_mapping.xlsx in your current working directory code folder"
        )

        print("Uploading latest disease mapping file to S3...")
        os.system(
            "aws s3 cp delt_mesh_mapping_temp.xlsx s3://{}/clinical-data-lake/uploads/FINAL_MESH_MAPPING/temp_mesh_mapping_folder/".format(
                s3_bucket_name
            )
        )
...
if __name__ == "__main__":
    Disease_std = DiseaseStandardizationProposed()
