...
class DrugStandardization(object):
    ...
    def main(self):
       ...
       # config_data = drug_std.load_json(drug_std_config_file_location)
        ...
        nlp_drug = '/clinical_design_center/data_management/cdl_sdo/qa/code/nlp_drug.csv'

        citeline_drugs = '/clinical_design_center/data_management/cdl_sdo/qa/code/' \
                         'ispharmadrug_true_ctln.csv'

        if os.path.exists(nlp_drug):
            os.system('rm ' + nlp_drug)
        os.system('rm -r /clinical_design_center/data_management/cdl_sdo/qa/code/delta_drug')
        os.system('hadoop fs -copyToLocal /user/hive/warehouse/delta_drug '
                  '/clinical_design_center/data_management/cdl_sdo/qa/code/')

        os.system('mv /clinical_design_center/data_management/cdl_sdo/qa/code/delta_drug/*.csv '
                  '/clinical_design_center/data_management/cdl_sdo/qa/code/nlp_drug.csv')
            ...
            #os.system('aws s3 cp s3://aws-a0220-use1-00-q-s3b-shrd-shr-data01/clinical-data-lake/'
                      #'temp/drug_std/dqs_drugs.csv ./')

        if os.path.exists(citeline_drugs):
            os.system('rm ' + citeline_drugs)
        os.system('rm -r /clinical_design_center/data_management/cdl_sdo/qa/code/citeline_drugs')
        os.system('hadoop fs -copyToLocal /user/hive/warehouse/citeline_drugs '
                  '/clinical_design_center/data_management/cdl_sdo/qa/code/')

        os.system('mv /clinical_design_center/data_management/cdl_sdo/qa/code/citeline_drugs/*.csv '
                  '/clinical_design_center/data_management/cdl_sdo/qa/code/ispharmadrug_true_ctln.csv')
        ...
        try:
            nlp_df=pd.read_csv("nlp_drug.csv", error_bad_lines=False)

        except Exception as e:
            print(e)

        nlp_df = self.process_aact_data(nlp_df)
        ...
        if dqs_flag == 1:
            merged_ctln = self.merge_combination_to_citeline(nlp_df, ctln_df, dqs_flag, dqs_df)
        elif dqs_flag == 0:
            merged_ctln = self.merge_combination_to_citeline(nlp_df, ctln_df, dqs_flag)
        ...
        nlp_ctln_df = self.match(nlp_df, ctln_df2_one_df, "processed")

        logging.info("Getting required columns from input dataframes..")


        ## get original column names of input and source df
        temp1 = self.get_org_cols(nlp_ctln_df, nlp_df, merged_ctln, dqs_flag)

        nlp_below_threshold = temp1[temp1['similarity']<THRESHOLD_SCORE].copy()
        ...
        if dqs_flag == 1:
            ...
            nlp_below_threshold.to_csv("not_matched_nlp.csv", index=False)

        elif dqs_flag == 0:
            logging.info("writing not mapped nlp drugs to CSV...")
        ...
        os.system('aws s3 cp drug_mapping_file.csv '
                  's3://aws-a0220-use1-00-q-s3b-shrd-shr-data01/clinical-data-lake/uploads/SDO_UPLOADS'
                  '/DRUG_MAPPING_FILE_LATEST/')
        os.system('aws s3 cp not_matched_aact.csv  '
                  's3://aws-a0220-use1-00-q-s3b-shrd-shr-data01/clinical-data-lake/uploads/SDO_UPLOADS'
                  '/NOT_MATCHED_DRUGS/')
