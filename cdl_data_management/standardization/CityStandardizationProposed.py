...
class CityStandardizationProposed:

    def ngrams(self, string, n=3):
        """
        returns n-grams of input city after some preprocessing on it
    ...
    def get_org_cols(self, df, input_df, std_df):
        """
        get original input city column and ctln df index
        df: disease source similarity dataframe
        input_df: aact or dqs cities dataframe
        std_df: base std df
        """
        ## can add here code to trim spaces on indexes columns

        # fixed some missing input cities value here by keeping input df on left
    ...
    def get_mapping_file(self, df, base_std_df):
        ...
        logging.info("Creating mapping for raw data...")
    ...
    def main(self):
        ...
        delta_file_path = CommonConstants.AIRFLOW_CODE_PATH + "/City_Delta"
        ...
        os.system(
            "hadoop fs -copyToLocal /user/hive/warehouse/City_Delta/ "
            + CommonConstants.AIRFLOW_CODE_PATH
            + "/"
        )
        city_file_path = CommonConstants.AIRFLOW_CODE_PATH + "/city_delta.csv"
        print(city_file_path)
        if os.path.exists(city_file_path):
            os.system("rm " + city_file_path)
        ...
        delta_df = pd.read_csv('city_delta.csv', sep='|', quoting=csv.QUOTE_NONE)

        mapping_file_path = CommonConstants.AIRFLOW_CODE_PATH + "/city_standardization.xlsx"
        if os.path.exists(mapping_file_path):
            os.system("rm " + mapping_file_path)

        os.system(
            'aws s3 cp s3://{}/clinical-data-lake/uploads/CITY/city_standardization/city_standardization.xlsx ./'.format(s3_bucket_name))
        mapping_file_df = pd.read_excel('city_standardization.xlsx')

        # Filling NaN values so re.sub doesn't give errors of not str like object
        delta_df.fillna('', inplace=True)
        delta_df.drop_duplicates(subset=['raw_city_name'], inplace=True)
        delta_df.rename(columns={'raw_city_name': 'processed'}, inplace=True)

        # adding indexes
        delta_df.insert(0, "delta_id", range(0, len(delta_df)))

        # extracting top 50 sponsor list
        mapping_list = mapping_file_df.city.unique().tolist()

        # Creating new dataframe with top pharma sponsors
        clean_df = pd.DataFrame(mapping_list, columns=['city'])

        clean_df.rename(columns={'city': 'processed'}, inplace=True)
        ...
        final_df = pd.merge(mapping_df, mapping_file_df, left_on='Standard City', right_on='city', how='left')
        final_df.drop(['map_to_idx', 'delta_id', 'processed', 'datasource', 'map_src_idx', 'city'], axis=1,
                      inplace=True)
        final_df.rename(columns={'City': 'raw_city_name', 'Standard City': 'city'},inplace=True)

        final_df.drop_duplicates(subset=['raw_city_name', 'city', 'standard_city', 'similarity'], inplace=True)
        final_df = final_df[['raw_city_name', 'city', 'standard_city', 'similarity']]
        final_df.to_excel("city_delta_ouput.xlsx", index=False)
        print("Output has been stored in xlsx file city_delta_ouput.xlsx in your current working directory code folder")

        print("Uploading latest city mapping file to S3...")
        os.system('aws s3 cp city_delta_ouput.xlsx s3://{}/clinical-data-lake/uploads/CITY/Temp_City_Mapping_Proposed_Holder/city_delta_ouput.xlsx'.format(s3_bucket_name))
        end_time = time.time()
