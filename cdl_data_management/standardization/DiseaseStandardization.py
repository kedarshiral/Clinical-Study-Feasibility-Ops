...
#   Predecessor module  :
#   Successor module    :   NA
#   Last changed on     :   09 Nov 2020
#   Last changed by     :   Sankarsh/Kuldeep
#   Reason for change   :
########################################################################################################################
...
class DiseaseStandardization(object):
    ...
    def standardize_ctms_diseases(self, disease_df):
        ...
        ctms_disease_df.to_csv("ctms_diseases.csv", index=False)
    ...
    def main(self):
        ...
        exploded_file = os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "/exploded.csv")
        synonym_file = os.path.join(
            CommonConstants.AIRFLOW_CODE_PATH, "/std_syn_one.csv"
        )
        delta_file = os.path.join(CommonConstants.AIRFLOW_CODE_PATH, "/delta.csv")
        ...
        final_data.rename(
            columns={
                "synonym": "Synonyms",
                "Standard_Names": "standard_disease",
        ...
        prev_mapping_file_location = (
            "s3://{}/clinical-data-lake/uploads/FINAL_DISEASE_MAPPING/Archive/".format(
                s3_bucket_name
            )
        )
        ...
        file_copy_command = "aws s3 cp {} {} --sse".format(
            ...
            os.path.join(
                prev_mapping_file_location, "disease_mapping_" + CURRENT_DATE + ".csv"
            ),
        )
        ...
        os.system(
            "aws s3 cp disease_mapping.csv s3://{}/clinical-data-lake/uploads/FINAL_DISEASE_MAPPING/".format(
                s3_bucket_name
            )
        )
