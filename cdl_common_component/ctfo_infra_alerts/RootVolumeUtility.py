...
def send_disk_usage_notification_email():
    ...
    try:
        ...
        email_template = "send_root_volume_notification_template.html"
        ...
        if count > 0:
            statement = ""
            if count == 1:
                statement = "Root volume has consumed more than " + str(
                    int(float(disk_usage_percentage))) + "% disk space, please see below utilization details"
            if count > 1:
                statement = "Root volume has consumed more than " + str(
                    int(float(disk_usage_percentage))) + "% disk space, please see below utilization details"
            ...
            disk_space_df = disk_space_df[disk_space_df[0] == root_dir]

            disk_space_df = disk_space_df.rename(
                columns={0: 'Filesystem', 1: 'Size', 2: 'Used', 3: 'Avail', 4: 'Use%', 5: 'Mounted'})

            html = disk_space_df.to_html(index=False)
            disk_name_df = pd.DataFrame(disk_name_list)
            disk_name_html = disk_name_df.to_html(index=False, header=False)
            replace_variables = {"statement": statement, "html": html}
