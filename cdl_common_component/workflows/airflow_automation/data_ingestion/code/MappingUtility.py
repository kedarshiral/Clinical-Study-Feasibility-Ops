...
class MappingUtility:
    ...
    def __init__(self):
        ...
        self.spark = PySparkUtility(execution_context).get_spark_context("MappingUtility",
        ...
        self.qc_query_path =str(self.configuration.get_configuration(
            [CommonConstants.ENVIRONMENT_PARAMS_KEY, "bucket_path"])) + "/uploads/QC_QUERIES/mapping_qc.xlsx"
    ...
    def call_mapping_utility(self, file_master_id, **kwargs):
        try:
            excel_df = self.spark.read.format('com.crealytics.spark.excel').option('header', 'true').load(
                self.qc_query_path)
            mapping_name = excel_df.select('Mapping_name').filter(excel_df.Query_id == str(file_master_id)).rdd.map(
                lambda x: x[0]).collect()
            description = excel_df.select('Description').filter(excel_df.Query_id == str(file_master_id)).rdd.map(
                lambda x: x[0]).collect()
            for each_row in excel_df.collect():
                if str(each_row.Query_id) == str(file_master_id):
            ...
            for each_df in mapping_name:
                ...
                if cmd_count == 1:
                    ...
                    email_subject = "Mapping validation"
...
if __name__ == "__main__":
    try:
        ...
        new_obj.call_mapping_utility(file_master_id)
    except Exception as e:
        logger.error(str(e))
        raise e
