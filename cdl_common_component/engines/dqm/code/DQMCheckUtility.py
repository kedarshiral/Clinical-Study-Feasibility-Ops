...
class DQMCheckUtility:
    ...
    def save_dqm_error_summary(
        ...
        try:
            ...
            logger.info('S3 Path error: ' + str(dqm_s3_error_location))
            ...
            logger.info('S3 Path summary: ' + str(dqm_s3_summary_location))


            if(error_union_df.rdd.isEmpty() == False):
                try:
                    ...
                    logger.info('Failed to write error_union_df \n' + str(e))
            else:
                logger.info('TEST: error_union_df is empty hence didnt write')


            if(summary_union_df.rdd.isEmpty() == False):
                try:
                    summary_union_df.coalesce(1).write.mode('overwrite'
                            ).format('parquet'
                                    ).partitionBy(CommonConstants.PARTITION_FILE_ID).save(dqm_s3_summary_location
                            + '/' + CommonConstants.PARTITION_BATCH_ID + '='
                            + str(batch_id))
                except Exception as e:
                    logger.info('Failed to write summary_union_df \n' + str(e))
            else:
                logger.info('TEST: summary_union_df is empty hence didnt write')
