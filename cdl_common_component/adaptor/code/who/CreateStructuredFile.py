...
"""
  ...
  Pre-requisites      :
  Last changed on     :   27th August 2018
  Last changed by     :   Himanshu Khatri
  Reason for change   :   Comments and formatting
"""
#this is the correct CreateStructuredFile.py file for end to end run
import getopt
import json
...
who_schema = StructType([StructField("TrialID", StringType(), True),StructField("date_of_export", StringType(), True),StructField("SecondaryIDs", StringType(), True),StructField("public_title", StringType(), True),StructField("Scientific_title", StringType(), True),StructField("url", StringType(), True),StructField("Public_Contact_Firstname", StringType(), True),StructField("Public_Contact_Lastname", StringType(), True),StructField("Public_Contact_Address", StringType(), True),StructField("Public_Contact_Email", StringType(), True),StructField("Public_Contact_Tel", StringType(), True),StructField("Public_Contact_Affiliation", StringType(), True),StructField("Scientific_Contact_Firstname", StringType(), True),StructField("Scientific_Contact_Lastname", StringType(), True),StructField("Scientific_Contact_Address", StringType(), True),StructField("Scientific_Contact_Email", StringType(), True),StructField("Scientific_Contact_Tel", StringType(), True),StructField("Scientific_Contact_Affiliation", StringType(), True),StructField("study_type", StringType(), True),StructField("study_design", StringType(), True),StructField("phase", StringType(), True),StructField("Date_registration", StringType(), True),StructField("Date_enrollement", StringType(), True),StructField("Target_size", StringType(), True),StructField("Recruitment_status", StringType(), True),StructField("Primary_sponsor", StringType(), True),StructField("Secondary_sponsors", StringType(), True),StructField("Source_Support", StringType(), True),StructField("Countries", StringType(), True),StructField("Conditions", StringType(), True),StructField("Interventions", StringType(), True),StructField("Agemin", StringType(), True),StructField("Agemax", StringType(), True),StructField("Gender", StringType(), True),StructField("Inclusion_Criteria", StringType(), True),StructField("Exclusion_Criteria", StringType(), True),StructField("Primary_Outcome", StringType(), True),StructField("Secondary_Outcomes", StringType(), True),StructField("Bridging_flag", StringType(), True),StructField("Bridged_type", StringType(), True),StructField("Childs", StringType(), True),StructField("type_enrolment", StringType(), True),StructField("Retrospective_flag", StringType(), True),StructField("results_actual_enrollment", StringType(), True),StructField("results_url_link", StringType(), True),StructField("results_summary", StringType(), True),StructField("results_date_posted", StringType(), True),StructField("results_date_first_publication", StringType(), True),StructField("results_baseline_char", StringType(), True),StructField("results_participant_flow", StringType(), True),StructField("results_adverse_events", StringType(), True),StructField("results_outcome_measures", StringType(), True),StructField("results_url_protocol", StringType(), True),StructField("results_IPD_plan", StringType(), True),StructField("results_IPD_description", StringType(), True),StructField("results_date_completed", StringType(), True),StructField("results_yes_no", StringType(), True),StructField("Ethics_Status", StringType(), True),StructField("Ethics_Approval_Date", StringType(), True),StructField("Ethics_Contact_Name", StringType(), True),StructField("Ethics_Contact_Address", StringType(), True),StructField("Ethics_Contact_Phone", StringType(), True),StructField("Ethics_Contact_Email", StringType(), True)])
...
class CreateStructuredFile:
    ...
    def __init__(self, access_key=None, secret_key=None):
        ...
        self.stitching_flag = None
        self.stitching_columns = None
        self.step_name = None
    ...
    def initialize_variables(self, input_config):
        ...
        try:
            ...
            if "stitching_flag" in input_config :
                self.stitching_flag = input_config["stitching_flag"]
            else :
                self.stitching_flag = "N"
            if "stitching_columns" in input_config :
                self.stitching_columns = input_config["stitching_columns"]
            else :
                self.stitching_columns = ""
            if "step_name" in input_config :
    ...
    def create_investigator_file_for_who(self, table_name, target_path):
        try:
            ...
            investigator_query = """select ROW_NUMBER() OVER (ORDER BY (Select 1)) AS id,trim(TrialID) as TrialID, trim(pn.CONTACT_FIRSTNAME) as Contact_Firstname, trim(pn.Contact_Lastname) as Contact_Lastname, trim(pn.Contact_Tel) as Contact_Tel,
                trim(pn.Contact_Email) as Contact_Email, trim(Contact_Address) as Contact_Address,trim(Contact_Affiliation) as Contact_Affiliation from (select concat_ws('\|',collect_set(trialid)) as trialid,
                ...
                posexplode(split(coalesce(Contact_Email,'NA'),';'))pn as p1, Contact_Email lateral view
                posexplode(split(coalesce(Contact_Lastname,'NA'),';'))pn as p2,Contact_Lastname lateral view
                posexplode(split(coalesce(Contact_Tel,'NA'), ';'))pn as p3,Contact_Tel lateral view
                posexplode(split(coalesce(CONTACT_FIRSTNAME,'NA'),';'))pn as p4,CONTACT_FIRSTNAME
    ...
    def execute_spark_query(self):
        ...
        try:
            ...
            if self.data_source == "clinicaltrials_gov":
                ...
                if type(self.s3_input_data_path) == list:
                    for s3_path in self.s3_input_data_path:
                        ...
                        temp_df = self.spark.read.format('csv').option('header','false').option('multiline','True').\
                            option('delimiter',',').option('quoteAll','True').option('ignoreTrailingWhiteSpace','True').\
                            option('escape','"').schema(who_schema).load(self.s3_input_data_path)
                        # merge dataframes for multiple files
                        if raw_data_frame:
                            raw_data_frame = raw_data_frame.union(temp_df)

                            for i in raw_data_frame.columns:
                                raw_data_frame = raw_data_frame.withColumn(i, regexp_replace(i, '"', ''))
                        else:
                    ...
                    raw_data_frame = self.spark.read.format('csv').option('header','false').option('multiline','True').\
                            option('delimiter',',').option('quoteAll','True').option('ignoreTrailingWhiteSpace','True').\
                            option('escape','"').schema(who_schema).load(self.s3_input_data_path)
            ...
            history_publish_data_location = None
            if self.stitching_flag == 'Y' :
                unique_column_names = self.stitching_columns
                history_publish_data_location = CommonUtils().fetch_successful_parquet_path_for_adapter(self.data_source, self.step_name)

            formatted_df.createOrReplaceTempView(temp_table_name)

            if history_publish_data_location is not None and self.stitching_flag == 'Y':
    ...
    def main(self, input_json_file_path):
        ...
        try:
            ...
            for file in parquet_files:
                if file.endswith(".parquet"):
                    logging.info("File copying is " + str(file))
                    command = "aws  s3 cp " + file + " " + parquet_output_path
                    logging.info("command for awscli ---->>> " + command)
                    CommonUtils().execute_shell_command(command)
