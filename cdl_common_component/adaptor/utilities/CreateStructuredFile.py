...
def parse_struct_of_struct_indications(data, parent, child,col_name=None):
    """
    ...
    if data:
        for parent_struct in data:
            ...
            for child_struct in child_list:
                ...
                for ele in val:
                    elem = ele[col_name]
                    output = "{}{}".format(output, elem) if first_subelem == 1 else "{};{}".format \
                        (output, elem)
                    first_subelem = 0
    return output
...
def parse_to_array(main_key):
    try:
        main_key = json.dumps(main_key)
    except:
        print("Inside Except of dumps")
    try:
        main_key = json.loads(main_key)
    except:
        print("Inside Except of loads")
    return main_key


def parse_biomarkers_caption(key):
    return 'Not Available' if key is None or len(key) == 0 else "|".join(str(e) for e in key)


def parse_biomarkers(main_key, key, key2) :
    res = list(find(key, main_key))
    final_list = []
    if res is None or len(res) == 0:
        final_list.append('Not Available')
    else:
        for i in range(len(res)):
            res_new = list(find(key2, res[i]))
            if res_new is None or len(res_new) == 0:
                final_list.append('Not Available')
            else :
                for i in range(len(res_new)):
                    if res_new[i] is None :
                        final_list.append('Not Available')
                    elif len(res_new[i])== 0:
                        final_list.append('Not Available')
                    else :
                        res_new_final= "^".join(str(e) for e in res_new[i])
                        final_list.append(res_new_final)
    if len(final_list) == 0 :
        final_res = 'Not Available'
    else:
       final_res = "|".join(str(e) for e in final_list)
    return final_res


# UDF: parse directMechanism
def parse_dm_drug_trends_years(main_key, key):
    res = list(find(key, main_key))
    return 'Not Available' if len(res) == 0 else "|".join(str(e) for e in res)

def parse_drug_trends(main_key, key):
    res = list(find(key, main_key))
    return 'Not Available' if len(res) == 0 else "^".join(str(e) for e in res)

def parse_iterate_dict(main_key, key, key2) :
    res = list(find(key, main_key))
    final_list = []
    for i in range(len(res)):
        a = parse_drug_trends(res[i], key2)
        final_list.append(a)
    return 'Not Available' if len(final_list) == 0 else "|".join(str(e) for e in final_list)

def parse_drugtrends_years(main_key,sub_key,key=None):
    try:
        main_key = json.loads(main_key)
    except:
        print("String type")
    ...
    inner_str = ""
    outer_str = ""
    for each_data in main_key:
        if sub_key not in each_data.keys():
            continue
        else:
            data = each_data[sub_key]
        if type(data)==list:
            for elem in data:
                if key not in elem.keys():
                    continue
                else:
                    inner_list.append(str(elem[key]))
            # inner_list = [str(elem[key]) for elem in data]
            inner_str = "; ".join(inner_list)
            outer_list.append(inner_str)
        else:
            outer_list.append(str(data))
    outer_str="|".join(outer_list)
    if outer_str=="" or outer_str=="|" or outer_str==";" or outer_str.replace('|','')=='':
        return None
    if outer_str[0] == '|' or outer_str[0] == ';':
        outer_str=outer_str[1:]
    if outer_str[-1] == '|' or outer_str[-1] == ';':
        outer_str=outer_str[:-1]
    return outer_str
...
class CreateStructuredFile:
    ...
    def initialize_variables(self, input_config):
        ...
        try:
            ...
            if (self.data_source == "eudract"):
                ...
                self.cycle_id = input_config["cycle_id"]
                self.csv_temp_path = input_config["csv_temp_path"]
                self.csv_temp_path = os.path.join(self.csv_temp_path, self.cycle_id)
                # to remove data from s3 from cycle_id
                self.csv_temp_path_till_trigger_id = self.csv_temp_path
                self.csv_temp_path = os.path.join(self.csv_temp_path, self.payload_id)
                self.parquet_temp_path = os.path.join(self.parquet_temp_path, self.cycle_id)
                ...
                parquet_file_name = "eudract_" + input_config[
                    DISEASE_AREA_COL_VALUE_KEY] + "_" + self.cycle_id + "_" + self.payload_id + ".parquet"
    ...
    def execute_spark_query(self):
        ...
        try:
            ...
            self.sqlContext.udf.register("parse_struct_of_struct_indications", parse_struct_of_struct_indications)
            self.sqlContext.udf.register("parse_dm_drug_trends_years", parse_dm_drug_trends_years)
            self.sqlContext.udf.register("parse_drug_trends", parse_drug_trends)
            self.sqlContext.udf.register("parse_iterate_dict", parse_iterate_dict)
            self.sqlContext.udf.register("parse_drugtrends_years", parse_drugtrends_years)
            self.sqlContext.udf.register("parse_to_array", parse_to_array)
            self.sqlContext.udf.register("parse_biomarkers_caption", parse_biomarkers_caption)
            self.sqlContext.udf.register("parse_biomarkers", parse_biomarkers)
