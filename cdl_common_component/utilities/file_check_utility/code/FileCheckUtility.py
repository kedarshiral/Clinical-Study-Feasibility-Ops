    ...
    def perform_header_check(self, file_complete_path=None, field_delimiter=None, column_name_list=None, file_format=None, file_name=None):
        ...
        try:
            ...
            if (file_format is not None) and (file_format.lower() == CommonConstants.PARQUET):
                ...
                file_df = spark_context.read.format('com.crealytics.spark.excel').option('header', 'true').load(
                    file_name_path)
                # file_df = spark_context.read.format('com.crealytics.spark.excel').option('header', 'true').option("sheetName", "Sheet1")
                #     .load(file_name_path)
