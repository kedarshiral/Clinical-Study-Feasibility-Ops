...
Tech Description    : This module is used for adding new log handlers if none exists in the
                     current logging context. Currently configured for console and file log
                     handlers. This module will be used by all python codes to have a
...
FILE_HANDLER_LOG_NAME = "CopyRedshiftttoS3.log"
...
def get_logger(log_level=None, enable_console_log=CONSOLE_HANDLER_ENABLED,
               enable_file_log=FILE_HANDLER_ENABLED,
               file_log_dir=FILE_HANDLER_LOG_DIR, file_log_name=FILE_HANDLER_LOG_NAME,
               rotational_handler_flag=None):
    """
    Purpose   :   If no existing log handler is found it creates new log handlers (console and
                file  handlers supported for NOW). It then sets the logging level of the handler
                to INFO/WARN/DEBUG
    Input     :   Optional log level - DEBUG/INFO/WARNING/ERROR, Optional flag to enable
                console logging - True/False Optional flag to enable file logging - True/False,
                Optional file log directory path Optional log file name
    ...
    if not root_logger.handlers:
        ...
        if enable_file_log:
            try:
                ...
                file_handler = TimedRotatingFileHandler(log_file_path, when='D', interval=1,
                                                        delay=True)
