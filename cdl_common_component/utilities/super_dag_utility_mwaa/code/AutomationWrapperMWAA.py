...
from DagTriggerUtilityMWAA import DagTriggerUtility
            ...
            if enable_flag.lower() == "y":
                if data_date is not None:
                    ...
                    try:
                        ...
                        trigger_dag.execute_dag(dag_id=dag_id, airflow_environment=env_configs.get_configuration(
                            [CommonConstants.ENVIRONMENT_PARAMS_KEY,"airflow_environment"]), sync=True)
                    except:
...
def prepare_config_file(config_file_name, automated_dag_name):
    try:
        template_file_name =  automated_dag_name
        # CommonConstants.AIRFLOW_CODE_PATH+"/" + "DataEngineering-L1-L2-A1-Super-Dag-Weekly-Full.template"
