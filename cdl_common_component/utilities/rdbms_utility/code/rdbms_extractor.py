...
_log_file_name__ = CommonConstants.RDBMS_LOG_FILE_NAME
__log_path__ = CommonConstants.RDBMS_LOG_PATH
__log_level__ = CommonConstants.RDBMS_LOG_LEVEL
__handler_type__ = CommonConstants.RDBMS_HANDLER_TYPE
__max_bytes__ = CommonConstants.RDBMS_MAX_BYTES
__backup_count__ = CommonConstants.RDBMS_BACKUP_COUNT
STATUS_FAILED = CommonConstants.RDBMS_FAILED_KEY
STATUS_SUCCEEDED = CommonConstants.RDBMS_STATUS_SUCCEEDED
...
class Utility(object):
    ...
    def rds_execute_query(self, cycle_id, src_system, status, step_name, database_name, object_name,
        ...
        """
        try:
            table = CommonConstants.RDBMS_LOG_DB_TBL
...
class Database_extractor(object):
    ...
    def save_to_s3(self, cycle_id, dataframe, file_name, src_system, status, step_name, database_name, object_name,
                   ...
                   query_description, prev_max_refresh_timestamp, curr_max_refresh_timestamp, start_time, overide_flag,date_format,timestamp_format,geography_id):
        """
    ...
    def read_from_database(self, query):
        ...
        try:
            ...
            # for each data inside ctl table
            for each_data in final_list:
                try:
                    src_system=each_data['src_system'].lower()
                    rdbms_host, rdbms_port, rdbms_username, rdbms_password, rdbms_database, rdbms_service_name, date_format, timestamp_format=self.rdbms_connection_details(src_system)
                    if each_data['active_flag'].lower() == "y" and src_system in app_config['datasource_details']:
                        ...
                        target_directory = str(datetime.datetime.now(timezone.utc).strftime(str(app_config["datasource_details"][src_system]['misc_configs']['file_format'])))
                        target_location=each_data['target_location']+str(each_data['object_name'])+"_"+target_directory
                        changeDateproperty=app_config["datasource_details"][src_system]['misc_configs']['changeDatetoTimestamp']
                        if each_data['load_type'].lower() == "incremental" or each_data['load_type'].lower() == "delta":
                            # check the proper conf for incremental load
                            if each_data["query_file_name"] is None or each_data["query_file_name"] == "" or len(database_list) > 1 or len(table_list) > 1:
                            ...
                            query = file_reader(app_config['datasource_details'][src_system]['misc_configs']['path'], each_data['query_file_name'])
                            ...
                            for each_increment_col in multiple_increment_column:
                                ...
                                if each_data['override_last_refresh_date'] == "" or each_data['override_last_refresh_date'] is None:
                                    ...
                                    if get_prev_max_time[each_increment_col] == 'None':
                                        query = query.replace("'$$" + each_increment_col+"'","null").replace("$$database_name",str(each_data['database_name']))
                                    else:
                                        get_timestamp_to_date = str(get_prev_max_time[each_increment_col])
                                        date_to_fetch_increment = get_timestamp_to_date.split(" ")
                                        query = query.replace("$$" + each_increment_col,date_to_fetch_increment[0]).replace("$$database_name", str(
                                    ...
                                    if get_current_max_time[each_increment_col] == 'None':
                                        ...
                                        query = query.replace("$$" + each_increment_col,str(get_current_max_time[each_increment_col])).replace("$$database_name",str(each_data['database_name']))
                            ...
                            query_to_update = "update {table_name} set override_last_refresh_date = Null where object_name='{object_name}';".format(
                                table_name=CommonConstants.RDBMS_CTL_DB_TBL, object_name=each_data['object_name'])
                        ...
                        self.save_to_s3(cycle_id=cycle_id, dataframe=df, file_name=each_data['query_file_name'],
                                            ...
                                            output_file_format=each_data['output_file_format'], overide_flag=overide_flag,date_format=date_format,timestamp_format=date_format,geography_id=each_data['geography_id'])
                except Exception as e:
                    ...
                    Utility().rds_execute_query(cycle_id=cycle_id,
                                                query_file_name=str(each_data['query_file_name']).split(".")[0],
                                                status=STATUS_FAILED,
                                                step_name="Failed to extract the data",
                                                start_time=start_time, end_time=end_time,
                                                target_location=target_location,
                                                src_system=each_data['src_system'],
                                                query_description=query,
                                                ...
                                                overide_flag=overide_flag, record_count="",geography_id=each_data['geography_id'])
...
if __name__ == '__main__':
    try:
        ...
        spark_handler_obj = Database_extractor()
        spark_handler_obj.read_from_database(query)
    except Exception as e:
